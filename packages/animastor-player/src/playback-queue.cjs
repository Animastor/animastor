...
//
// assets-state is a SHARED endpoint: the Generate page gates on it as much
...
module.exports = function(app, ctx, deps) {
    const { getAllChunks, getBookWindowStatus, layerConfig, sceneAssetsRepo, computeIuReady } = deps;
    ...
    app.get('/api/v1/book/:bookId/chunks', async (req, res) => {
        ...
        // TTS pipeline may create multiple chunks per scene (_0001, _0002,
        // _0003) for long text segments — they all serve the same merged
        // audio file. Keep only the first chunk (lowest chunk_index) per
        // (chapter_id, scene_id).
        const dedupedIds = [];
        ...
        if (allIds.length > 0) {
            try {
                ...
                const raw = await ctx.redis.mget(keys);
    ...
    // ── ASSETS STATE (readiness — shared with Generator gating) ──────────
    app.get('/api/v1/book/:bookId/assets-state', async (req, res) => {
        try {
            ...
            if (allChunkIds.length > 0) {
                try {
                    const raw = await ctx.redis.mget(allChunkIds.map(id => `animastor:chunk:${id}`));
                    ...
                    for (const cid of allChunkIds) {
                        try { const c = await ctx.getChunk(cid); if (c) chunkById.set(cid, c); } catch (_) {}
                    }
                }
            }
            ...
            try {
                ...
                if (buildId) {
                    ...
                    for (const { chapter_id: ch, scene_id: sc } of uniqueScenes.values()) {
                        const rows = await ctx.iuRepo.getImageUnitsForScene(buildId, bookId, ch, sc);
                        if (rows.length > 0) {
                            scopeIuTotal += rows.length;
                            scopeIuReady += await computeIuReady(ctx.redis, sceneAssetsRepo, bookId, ch, sc, rows.length);
                        }
                    }

                    const layerCfg = await layerConfig.get(ctx.redis, bookId);
                    const imagesEnabled = layerCfg?.image_enabled !== false;
                    if (imagesEnabled) {
                        // Cover lookup reads through the Player boundary
                        // (playerModel → Canonical Book Model).
                        const coverCh = ctx.playerModel.loadBook(bookId)?.chapters?.find(ch => ch.type === 'cover');
                        if (coverCh && coverCh.scenes && coverCh.scenes.length > 0) {
                            const coverScene = coverCh.scenes[0];
                            // chapter_id for modern lazy-book chapters,
                            // `chapter` for legacy parse.js chapters.
                            const coverChapterId = coverCh.chapter_id ?? coverCh.chapter;
                            const coverSceneId = coverScene.scene_id;
                            const rows = await ctx.iuRepo.getImageUnitsForScene(buildId, bookId, coverChapterId, coverSceneId);
                            if (rows.length > 0) {
                                coverIuTotal = rows.length;
                                coverIuReady = await computeIuReady(ctx.redis, sceneAssetsRepo, bookId, coverChapterId, coverSceneId, rows.length);
                            } else {
                                ...
                                if (coverIuTotal > 0) {
                                    coverIuReady = await computeIuReady(ctx.redis, sceneAssetsRepo, bookId, coverChapterId, coverSceneId, allUnits.length);
                                }
                            }

                            if (coverIuReady < coverIuTotal && buildId) {
                                try {
                                    const buildDir = path.join(ctx.outputRoot, buildId);
                                    if (fs.existsSync(buildDir)) {
                                        const iuPrefix = naming.iuImagePrefix(bookId, coverChapterId, coverSceneId);

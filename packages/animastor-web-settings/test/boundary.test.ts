// Package boundary tests — verify that @animastor/web-settings never imports
...
describe('Package boundary — @animastor/web-settings', () => {
  ...
  it('src/ contains exactly the pinned inventory (index.ts + aiProviders.ts + localAi.ts)', () => {
    const srcFiles = readdirSync(SRC_DIR)
      .filter((f) => /\.(ts|tsx)$/.test(f))
      .sort();
    expect(srcFiles).toEqual(['aiProviders.ts', 'index.ts', 'localAi.ts']);
  });

  it('test/ contains exactly the pinned inventory (aiProviders.test.ts + boundary.test.ts + localAi.test.ts)', () => {
    const testFiles = readdirSync(TEST_DIR)
      .filter((f) => /\.(ts|tsx)$/.test(f))
      .sort();
    expect(testFiles).toEqual(['aiProviders.test.ts', 'boundary.test.ts', 'localAi.test.ts']);
  });
});

...
- **Runtime Result** (`src/runtime-result.js`, exported as `runtimeResult`) —
  the Phase 5 stable shape of a runtime job/dispatch execution result
  (`completed | failed | cancelled`), produced by the runtime tier and
  consumed by the orchestration tier (`@animastor/orchestration`), injected
  through the composition-root seam. Moved verbatim from
  `backend/src/contracts/runtime-result.js` at the §32.30 physical extraction
  (docs/architecture/PHASE_5_ORCHESTRATION_RUNTIME.md §3.1); a leaf module —
  no relative requires, never participates in an import cycle.
...
## What is inside (Runtime Result)

| Export | Meaning |
|---|---|
| `RUNTIME_RESULT_STATUS` / `RUNTIME_RESULT_STATUSES` | frozen status vocabulary: `completed`, `failed`, `cancelled` |
| `OUTCOME_TO_STATUS` / `statusFromOutcome` | dispatch-engine finalization outcome → contract status |
| `createRuntimeResult` | validated, frozen Runtime Result factory (`jobId` or `dispatchId` required) |
...
| gpu-hub | `require('@animastor/contracts').jobProtocolV2` — runtime import via optionalDependency or docker bind mount |
| orchestration | `require('@animastor/contracts')` root specifier — `.jobProtocolV2` (scene-orchestrator job ids) and `.runtimeResult` (`runtime-result-emitter` result contract); registry dependency `^0.1.0` |
| worker | Generated byte-exact copy (`worker/worker/job-protocol-v2.cjs`) synced by `worker/tools/sync-protocol.cjs` |
...
// or namespaced:
const { jobProtocolV2, runtimeResult } = require('@animastor/contracts');
```
...
- Publishable to npm registry as `@animastor/contracts` (see PHASE_10F audit).

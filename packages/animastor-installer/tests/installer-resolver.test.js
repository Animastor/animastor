...
function workerProbe(workerType) {
    return {
        ...
        bundle: {
            ...
            files: ['worker.cjs', 'worker-env.cjs', 'worker-cleanup.cjs', 'worker-cleanup-journal.cjs', 'job-protocol-v2.cjs', 'package.json', 'package-lock.json', '.env.example'],
        },
...
describe('Compatibility Resolver', () => {
    ...
    describe('scenario 4: existing compatible ComfyUI', () => {
        it('reports everything installed against the known-working reference', () => {
            ...
            expect(entry(r, 'model:image.vae.qwen-image-vae').grade).to.equal('checksum-verified');

...
- appearance: DETAILED PHYSICAL appearance ONLY — age, face, hair, eyes, build, expression. DO NOT include clothing here — clothing belongs in the separate `clothes` field below. This is CRITICAL — must be a vivid visual description like an author wrote it, 2-4 sentences. **MUST be written in ENGLISH (en)** — it is injected verbatim into English-only image/video prompts, so never write it in the book's language, even if the source text is in another language. **As a default, describe from general to specific: overall build/silhouette → face → hair → eyes → expression → small distinguishing details** — do not hop randomly between body parts, since the description becomes one block of the image prompt. A model-specific Prompt Profile may refine how the description is best ordered.

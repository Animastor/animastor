...
// ======================================================
// KNOWLEDGE BASE — still available but NOT included in prompts
// ======================================================
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory()) walkDir(fullPath, depth + 1);
                    ...
                    try {
                        ...
                        console.warn(`[AGENT] Failed to load example ${fullPath}: ${e.message}`);
                    }
                }
            }
        }
    ...
    if (fs.existsSync(rulesDir)) {
        for (const f of fs.readdirSync(rulesDir).filter(f => f.endsWith('.md'))) {
            try {
                ...
                console.warn(`[AGENT] Failed to load rule ${f}: ${e.message}`);
            }
        }
    }
    ...
    if (fs.existsSync(skillsDir)) {
        for (const f of fs.readdirSync(skillsDir).filter(f => f.endsWith('.md'))) {
            try {
                ...
                console.warn(`[AGENT] Failed to load skill ${f}: ${e.message}`);
            }
        }
    }

    console.log(`[AGENT] Knowledge base loaded: ${kb.loaded_files.length} files, ${kb.total_chars} chars (not used in prompts)`);
    return kb;
}

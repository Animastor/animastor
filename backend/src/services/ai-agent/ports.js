// ======================================================
// AI Agent — shared host-port mechanism
// ======================================================
// The AI Agent / AI Analysis contour
// (docs/architecture/ai-agent-contour-extraction-c21.md) executes every AI
// analysis task with the same mechanism:
//
...
function assertHostPorts(ports, required, taskName) {
    ...
    if (missing.length) {
        throw new Error(`${taskName}: missing host port(s): ${missing.join(', ')} — persistence and the LLM seam are host-injected (ai-agent contour, C21 §3)`);
    }
    return _ports;
}

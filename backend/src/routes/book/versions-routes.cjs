...
//
// ctx fields used here: { sceneAssetsRepo, log }

module.exports = function registerVersionRoutes(app, ctx) {
    const { sceneAssetsRepo, log } = ctx;
    ...
    app.get('/api/v1/book/:bookId/versions', async (req, res) => {
        try {
            ...
            // 1. Scene-level versions from scenes table (via scene-assets-repo)
            const sceneRows = await sceneAssetsRepo.getSceneVersions(bookId);

            // 2. Asset-level versions from scene_assets table (via scene-assets-repo)
            const assetRows = await sceneAssetsRepo.getSceneAssetVersions(bookId);

...
const ADAPTERS_REQUIRED = [
  ...
  '@animastor/web-player', // Player release/seek — Phase 3: package entry
  '../api/client',
...
// ── Physical extraction — File host boundary ──

// The composition root is the ONLY host place where the File package meets
...
const FILE_ADAPTERS_REQUIRED = [
  ...
  '../state/generateStore',
  '../state/fileStore',
  '@animastor/web-player', // Player release port — Phase 3: package entry
  '@animastor/web-file',
].sort();
...
describe('File contour guard (file-module-extraction-audit.md)', () => {
  ...
  it('Physical extraction — adapter wires all host infrastructure seams', () => {
    const specs = importSpecifiers(FILE_ADAPTERS);
  ...
  it('Physical extraction — no old File remnants: pages/FilePage.tsx deleted from host', () => {
    const fileKeys = Object.keys(RAW_SOURCES);
    expect(fileKeys).not.toContain('/src/pages/FilePage.tsx');
  });

  it('Physical extraction — no old File remnants: modules/file/ deleted from host', () => {
    const fileKeys = Object.keys(RAW_SOURCES);
    const fileModuleKeys = fileKeys.filter((k) => k.startsWith('/src/modules/file/'));
...
describe('B1 split guards — fileStore ownership + cycle dissolution (file-module-extraction-audit.md)', () => {
  ...
  it('Cycle guard (dissolved → inverted) — generateStore no longer imports playbackStore; the playbackStore → generateStore edge is now a PlayerPorts injection', () => {
    ...
    // Phase 3: playbackStore physically lives in @animastor/web-player
    ...
    // Phase 3: the Player engine lives in @animastor/web-player (package-side
    // boundary.test.ts pins that it imports no host module — the edge cannot
    // exist in host source anymore). The host leg reaches the package entry:
    expect(importSpecifiers(FILE_ADAPTERS)).toContain('@animastor/web-player');
  });
  ...
  it('Cycle guard — NO state-module cycle exists at all (was: one frozen cycle allowed)', () => {
    ...
    // Phase 3: playbackStore lives in the @animastor/web-player package, so it
    // is not even part of the host state-module graph scanned here.

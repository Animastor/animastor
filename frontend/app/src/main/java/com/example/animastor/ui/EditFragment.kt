...
class EditFragment : Fragment(R.layout.fragment_edit) {
        ...
        try {
            ...
            when (tab) {
                0 -> {
                    ...
                    val ch = chapters.getOrNull(currentChIndex)
                    if (ch != null) {
                        buildChapterTitleField(frame, ch)
                    }
    ...
    private fun saveToBackend() {
        try {
            ...
            val sc = currentScene()
            if (bd == null || sc == null) {
                showSaveError("No book or scene data")
                return
            }
            ...
            val scenes = modifiedCh.scenes?.toMutableList() ?: return
            ...
            lifecycleScope.launch {
                try {
                    ...
                    chapters = modifiedChapters.toList()
    ...
    private suspend fun loadTimelineData() {
        ...
        try {
            val waveformJob = lifecycleScope.launch {
                try {
                    waveformData = viewModel.repository.getSceneWaveform(bookId, ch.chapter ?: "", sc.scene_id ?: "", buildId)
                } catch (_: Exception) {
                    waveformData = null
                }
            }
            val timingsJob = lifecycleScope.launch {
                try {
                    timingData = viewModel.repository.getSceneTimings(bookId, ch.chapter ?: "", sc.scene_id ?: "", buildId)
                } catch (_: Exception) {
                    timingData = null
                }
            }
    ...
    private fun loadAudioFileSync() {
        ...
        lifecycleScope.launch {
            try {
                val bytes = viewModel.repository.getSceneAudio(bookId, ch.chapter ?: "", sc.scene_id ?: "", buildId)
                val file = File(requireContext().cacheDir, "scene-${bookId}-${ch.chapter}-${sc.scene_id}.mp3")

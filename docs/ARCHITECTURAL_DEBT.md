...
**Возможные последствия:**
- Нельзя переключиться между OpenRouter, NVIDIA, локальной моделью без изменений кода
- Отсутствие fallback цепочки (если qwen недоступен → попробовать GPT)
...
**Описание проблемы:** backend.cjs не обрабатывает сигналы SIGTERM/SIGINT для корректного завершения процессов, снятия аренд (leases), сохранения состояния.

**Причина возникновения:** Не было необходимости — проект запускается в Docker контейнерах.

**Возможные последствия:**
- Stale dispatch leases при перезапуске (частично решается очисткой при старте)
- Потеря незавершённых задач
- Долгое восстановление после рестарта

**Затрагиваемые компоненты:** backend.cjs, runtime-loop.js

---

## 14. Отсутствие метрик и мониторинга

**Описание проблемы:** Нет системы сбора метрик (Prometheus, OpenTelemetry, etc.). Есть только логгирование в console.log.

**Причина возникновения:** Проект на ранней стадии, мониторинг не был приоритетом.

**Возможные последствия:**
- Невозможность обнаружить проблемы до их критического проявления
- Отсутствие данных для оптимизации производительности

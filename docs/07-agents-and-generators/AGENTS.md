...
bootstrapWithAgent():
  ...
  runPipeline():
    ...
    Шаг 3: stepCreateScenes() — сцены (окно 3 сцены / 4000 символов)
...
**Ограничения:** Ровно 3 сцены за одно окно (`WINDOW_SIZE=3`), максимум 4000 символов текста (`MAX_WINDOW_CHARS=4000`).

**Промпт:** AI получает инструкцию создать **EXACTLY 3 scenes** с ограничением **~65 слов на сцену**
(≈20 секунд аудио при скорости русской речи). Если предложение заканчивается чуть за лимитом —
это нормально, предложение завершается. Естественные нарративные границы предпочтительнее равных
по длине частей. Заголовки глав в сцены не включаются — они добавляются програмно.
...
- Размер окна: 4000 символов / 3 сцены

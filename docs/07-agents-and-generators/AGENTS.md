...
**Ограничения:** До 8 сцен за одно окно (`MAX_SCENES_PER_CHUNK=8` — **жёсткий верхний предел, не целевое количество**),
текст сцены берётся из первых `SCENE_CHUNK_SIZE=1500` символов окна (reconnaissance использует все 4000).
...
4. **Максимум: ~30 секунд (~95 слов)** — жёсткий лимит. Сцена НИКОГДА не должна
   превышать этот порог. Если добавление следующего предложения превысит ~95 слов,
   закрыть сцену на предыдущем предложении.
...
7. **Verbatim coverage** — каждое слово исходного текста должно попасть ровно в одну
   сцену, дословно, без пропусков и перекрытий.

**ВАЖНО:** `MAX_SCENES_PER_CHUNK=8` — это жёсткий верхний предел, а не целевое
количество. Если текст естественно укладывается в 3-4 сцены, это правильно.
Не нужно заполнять все 8 сцен.

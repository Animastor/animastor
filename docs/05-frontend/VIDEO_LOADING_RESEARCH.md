...
11. **Android: persistent disk-cache для видео (Media3 SimpleCache +
    ...
    Известное ограничение: SimpleCache не делает ревалидацию, а бэкенд
    регенерирует файлы на месте (тот же URL, новые байты) — закэшированные
    диапазоны могут устареть при регенерации сцены; это отдельная задача
    (например, build_id в cache key).

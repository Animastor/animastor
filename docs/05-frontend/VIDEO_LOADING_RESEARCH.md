...
Базовый размер контента (~43 МБ видео на просмотренную сцену) не меняется — он
неизбежен, пока видео реально играет. Прогрессивный стриминг (faststart + Range,
варианты A–C) — отдельный следующий этап.

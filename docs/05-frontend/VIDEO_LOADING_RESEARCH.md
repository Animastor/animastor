...
Range** (первый кадр за ~1 с), повторные просмотры идут из медиа-кэша. Осталась
опциональная замена MediaPlayer на ExoPlayer/Media3 (более надёжный долгосрочный
плеер с кэшированием на диске через CacheDataSource, но большой риск для
выверенного seek-поведения).

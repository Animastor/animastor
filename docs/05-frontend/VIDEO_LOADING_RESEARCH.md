...
Range** — первый кадр за ~1 с вместо полной выкачки, видео не занимает память
и диск. Из вариантов A–C осталась опциональная замена MediaPlayer на
ExoPlayer/Media3 (более надёжный долгосрочный плеер, но большой риск для
выверенного seek-поведения) и HTTP-кэш-заголовки (Cache-Control/ETag) на
`streamFileWithRange` для повторных просмотров без повторной загрузки.

...
| **`layer-config`** | **Redis + book.json (durable copy)** | routes | which layers are enabled per book | ✅ (Brick №2 — Redis key restored from book.json on startup, C6) |
...
- Better: persist layer-config to PG or book JSON — **✅ IMPLEMENTED (Cathedral Brick №2, Aug 2026):** `layerConfig.set()` writes a durable copy of `layer_config` to `book.json`; C6 phase of `reconcileCycle` (every cycle) restores the Redis key from `book.json` (missing keys only, Redis remains source of truth during operation). The rebuild phase can read `book.json.layer_config` and respect disabled layers. See §9 finding 13.
...
**⚠️ Gap found during the analysis (not part of Operation #1) — ✅ FIXED (follow-up, Aug 2026):** `bootstrapNextWindow` — the *explicit* VBook "Continue" action — cleared stale `cancelled` agent-session statuses and the Redis `cancelled-workers` key ("user pressing = intent to CONTINUE", bootstrap.js:305-315), **but did not clear the `generation_cancellations` tombstone**. Consequence (before the fix): user cancels → presses "Continue" (new run starts) → Redis dies → backend restarts → `startup-resume` sees the tombstone and skips the book the user explicitly continued. **Fixed:** `generationCancelRepo.clear(bookId)` is now called in the "intent to CONTINUE" cleanup block of both `bootstrapNextWindow` and `bootstrapWithAgent` (best-effort, like `regenerate`), so every explicit-new-run path clears the tombstone consistently. Covered by `backend/tests/bootstrap-cancel-continue.test.js` (2 tests: Continue clears; book-not-found does not clear).
...
FOR each book b:
    ...
    layers_enabled = layer-config[b]  (Redis) → fallback DEFAULTS(all true); Brick №2:
                  Redis key restored from book.json on startup (C6) — §5.3 caveat closed
...
3. **✅ Constraint (4) — no regeneration of valid artifacts:** PG `ready` rows are insufficient on their own (files can be missing; C0 over-marks) — the predicate requires an **FS probe** + `hasRealAudio`. **Caveat (RESOLVED by Brick №2):** layer-config is now durable in `book.json` and restored to Redis on startup (C6) — books with disabled layers are no longer regenerated for disabled stages after Redis loss.
...
13. **✅ Persistent layer-config implemented (Brick №2, Aug 2026):**
    - `layerConfig.set()` duplicates config into `book.json` (`persistToBook`, best-effort, multi-file + legacy single-file formats). Single write point — `set()`; restoration — `restoreFromBooks()`.
    - C6 phase of `reconcileCycle` calls `layerConfig.restoreFromBooks(redis)` **every cycle** (not just startup): fills **only missing** Redis keys from durable copies (normalization via `normalize()`), live Redis state never overwritten. This also heals `cleanBookRedisKeys` key deletion on cache-clear / import / recovery, as long as the book exists — config does not disappear from Redis until restart.
    - `saveBookBundle` persists `layer_config` when overwriting book.json **in both branches**: serialize (`book.book`) and raw-files (bundle re-import) — editor or re-import does not overwrite the durable copy.
    - **Residual risk (best-effort by design):** read-modify-write without lock between `set()` and `saveBookBundle` theoretically allows lost-update (concurrent edit + PUT layer-config at same moment) — acceptable for local-first product, documented.
    - Covered by `backend/tests/layer-config-persistence.test.js` (10 tests: set→book.json, merge, restore-fill, restore-no-overwrite, legacy-format, saveBookBundle-preserve both branches). **Backend tests passing, 0 failing.**
    - **Closed chain for option E:** PostgreSQL → book.json (`layer_config` + `build_id`) → filesystem → deterministic WORK_TO_DO. Blocker §10.1 closed.
14. **✅ Two ACCEPT gaps closed (follow-up, Aug 2026):**
    - **Race `restoreFromBooks()` ↔ concurrent `set()`:** `restoreFromBooks` now writes **with a single atomic `SET key value NX`** (standard codebase primitive — dispatch-engine leases, scheduler tick lock, regenerate lock, dedup keys) instead of get-then-set. TOCTOU window where concurrent `set()` could be overwritten by stale durable copy is structurally excluded: if key already exists, NX returns `null` and live state is intact. Verified by spy test: **0 `get` calls, one atomic write per book**.
    - **`set()` with empty Redis (window after loss, before C6):** merge base is now durable copy from `book.json` (via `_readDurable`), not defaults. Otherwise a partial PUT (e.g., only `chunk_size`) in this window would silently reset previously saved `image_enabled=false` back to true and overwrite durable copy with defaults+partial.
    - **Integration test via REAL `reconcileCycle`** (`backend/tests/layer-config-reconcile.integration.test.js`): real cycle (not isolated C6), real layer-config service, real FS (tmp `BOOKS_DIR`), mock redis, remaining cycle phases use real modules. Four scenarios: (a) startup cycle restores key + `layer_config_restore:1` phase in `phases` + `summary.errors` empty; (b) periodic cycle heals missing key (cache-clear case); (c) live Redis state not overwritten (SETNX semantics via real cycle); (d) **idempotency**: second cycle — NX no-op, phase does not appear, key byte-identical (this is what makes C6-every-cycle safe).
    - **Intentional semantics (not a bug):** with durable base in `set()`, re-import of existing book now preserves `chunk_size`/timeouts from durable copy instead of resetting to defaults (body fields still win). This is a direct consequence of the brick's goal — config persistence.
15. **✅ WORK_TO_DO rebuild implemented (Option E, Aug 2026) + follow-up audit fixes:**
    - **C7** phase in `reconcileCycle` (`rebuildWorkList()`), **only on `startup:true`**, under `CLEANUP_LOCK`, after C6 (last state-writing C-phase — §6a design requirement).
    - §4 predicate: `getDirtyScenesByVersion` ∪ `dirty_unit_ids` ∪ `stale|failed|pending` in `scene_assets` ∪ FS probe `getSceneFilesStatus` ∪ `hasRealAudio` — **Redis states not read** (immunity to C0 over-marking, P1).
    - `build_id` pinned to `manifest.build_id` from `book.json` (fallback `'default'`); tombstone books skipped fail-closed (`getAllCancelled`); disabled layers do not get PENDING (layerCfg from Redis, restored by C6); **C7 itself dispatches nothing** — only the regular scheduler.
    - **Safe materialization (follow-up gap 1):** PENDING targets written ONLY via facade — `setScenePending` for NEW/DIRTY/FAILED (valid transitions) and two-step `markDirtyScene` (READY/PLACEHOLDER→DIRTY) → `setScenePending` (DIRTY→PENDING) for C0 over-mark. `unsafeRestoreAssetState` only for READY targets (restore-fact, guarded): `GENERATING`/`PENDING` current **never touched by recovery** → downgrade of in-progress generation structurally impossible (test: GENERATING not downgraded). S2.4 whitelist comment in `scene-state.js` updated.
    - **Dirty-intent actually affects predicate (follow-up gap 2):** previously `dirtyScenes` was collected but not consulted — `dirty_unit_ids` and per-asset statuses lost regeneration intent. Now: stale (version-mismatch ready) → PENDING; `scene_assets.status IN (stale,failed,pending)` per `asset_type` → PENDING even with valid leftover file; `dirty_unit_ids` → image PENDING (force-regen units).
    - **Disabled-layer semantics proven via real call graph (follow-up gap 3):** in `shouldScheduleAssets` `enabled = layerCfg.* !== false || isInFlight(state)` — for disabled layer with missing state: `false || isInFlight('new')` = **false** → never dispatched. Danger would exist if C7 wrote PENDING for disabled layer (isInFlight would fire) — C7 skips disabled stages, proven by test via real `shouldScheduleAssets`.
    - Covered by `backend/tests/worklist-rebuild.integration.test.js` (15 scenarios via real `reconcileCycle`/`rebuildWorkList`: half-gen, **C7 idempotency**, fully-valid no-op, cancelled skip, disabled-layer, version-stale, GENERATING-not-downgraded, C0 self-heal, PLACEHOLDER→PENDING, dirty_unit_ids force-regen, failed/pending re-dispatch, disabled-layer no-NEW→dispatch via real call graph, disabled-layer with PENDING → dispatch proof, **TOCTOU: concurrent tick between guard-read and write does not overwrite** (fresh-state re-read + retreat; in-flight GENERATING survives)). **Full suite: 1117 passing, 0 failing.**
    - **Intentional semantics:** the `worklist_rebuild:N` phase row counts **scenes** (scenes added), not books. `failed` row for image/video with existing file → READY (no re-dispatch) — this is a deliberate choice: failure may have occurred after artifact write; file validity determined by FS probe `getSceneFilesStatus` (version-stale gate applied to `ready` rows). `hasRealAudio` probe failure logged as warn and fail-closed treats audio as invalid (PENDING), not swallowing the error silently.
    - **Isolation note:** `purgeMockedModules` + module re-bind in `before()` — order-dependent workaround against pollution from happy-path.test.js / scope-slide.test.js (require.cache for runtime-config). Safe as long as file remains last alphabetically ('w'); adding files after 'w' requires workaround review (durable fix — restore cache in happy-path/scope-slide themselves).
    - Design doc updated (§12 Implementation status).
...
2. **✅ DONE (Brick №2, Aug 2026):** layer-config now persists in `book.json` on every `set()` and restored to Redis on startup (C6) — rebuild can respect disabled layers from `book.json.layer_config`. PG schema not changed.
3. **✅ DONE (Option E, Aug 2026):** the rebuild is implemented as phase C7 (see §9 finding 15) — a generalization of C5 using the §4 predicate, under `CLEANUP_LOCK`, startup-only, writing per-asset states + `SADD` only.
...
6. **Two decisions from the Operation #1 follow-up (Aug 2026) — both resolved:**
   - **(a) ✅ `bootstrapNextWindow` tombstone clear implemented** (§5.4b): `generationCancelRepo.clear(bookId)` added to the "intent to CONTINUE" cleanup blocks of `bootstrapNextWindow` and `bootstrapWithAgent` in `agent/bootstrap.js` — every explicit-new-run path now clears the tombstone consistently with `regenerate`. Covered by `backend/tests/bootstrap-cancel-continue.test.js`.
   - **(b) ✅ Best-effort cancel retained** (§5.4c): decision made — keep the best-effort tombstone write. Rationale: cancel must always succeed even when PG is down; the two other PG writes in the cancel path (`cancelActiveTasksForBook`, `agent_sessions` update) are also best-effort, so making only the tombstone fail-closed would be inconsistent; and the residual race needs three sequential failures. Revisit with option C (write-ahead) only if cancellation must become an absolute invariant (multi-user future).

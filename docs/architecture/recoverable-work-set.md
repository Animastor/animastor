...
15. **✅ WORK_TO_DO rebuild implemented (Option E, Aug 2026):**
    ...
    - `build_id` пинится к `manifest.build_id` из `book.json` (fallback `'default'`); tombstone-книги пропускаются fail-closed (`getAllCancelled`); отключённые слои не получают PENDING (layerCfg из Redis, восстановленный C6); состояния материализуются (`unsafeRestoreAssetStates`, batch) ДО `SADD` в active-scenes; **сам C7 ничего не диспатчит** — только обычный scheduler.
    - Покрыто `backend/tests/worklist-rebuild.integration.test.js` (6 сценариев через реальный `reconcileCycle`: half-gen, **идемпотентность C7 (второй startup-цикл — no-op)**, fully-valid no-op, cancelled skip, disabled-layer, version-stale). **Full suite: 1107 passing, 0 failing.**

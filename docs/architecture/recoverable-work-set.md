...
14. **✅ Два ACCEPT-пробела закрыты (follow-up, Aug 2026):**
    - **Race `restoreFromBooks()` ↔ concurrent `set()`:** `restoreFromBooks` теперь пишет **одним атомарным `SET key value NX`** (стандартный примитив кодовой базы — dispatch-engine лейзы, scheduler tick lock, regenerate lock, dedup-ключи) вместо get-then-set. TOCTOU-окно, в котором concurrent `set()` мог быть затёрт устаревшей durable-копией, структурно исключено: если ключ уже появился, NX вернёт `null` и живое состояние цело. Проверяется тестом со спаем: **0 вызовов `get`, одна атомарная запись на книгу**.
    - **`set()` при пустом Redis (окно после потери, до C6):** база для merge теперь — durable-копия из `book.json` (через `_readDurable`), а не defaults. Иначе частичный PUT (например, только `chunk_size`) в этом окне молча сбросил бы ранее сохранённое `image_enabled=false` обратно в true и перезаписал бы durable-копию defaults+partial.
    - **Integration test через РЕАЛЬНЫЙ `reconcileCycle`** (`backend/tests/layer-config-reconcile.integration.test.js`): настоящий цикл (не изолированная C6), реальный layer-config-сервис, реальная FS (tmp `BOOKS_DIR`), mock redis, остальные фазы цикла работают на реальных модулях. Четыре сценария: (a) startup-цикл восстанавливает ключ + фаза `layer_config_restore:1` в `phases` + `summary.errors` пуст; (b) периодический цикл heal'ит пропавший ключ (cache-clear кейс); (c) живое Redis-состояние не перетирается (SETNX-семантика через реальный цикл); (d) **идемпотентность**: второй цикл — NX no-op, фаза не появляется, ключ байт-в-байт не изменился (это и делает C6-каждый-цикл безопасным).
    - **Intentional семантика (не баг):** с durable-базой в `set()` ре-импорт существующей книги теперь сохраняет `chunk_size`/таймауты из durable-копии вместо сброса в defaults (поля body по-прежнему побеждают). Это прямое следствие цели кирпича — персистентность конфига.
15. **✅ WORK_TO_DO rebuild implemented (Option E, Aug 2026) + follow-up audit fixes:**
    - Фаза **C7** в `reconcileCycle` (`rebuildWorkList()`), **только на `startup:true`**, под `CLEANUP_LOCK`, после C6 (последняя state-writing C-фаза — требование §6a дизайна).
    - Предикат §4: `getDirtyScenesByVersion` ∪ `dirty_unit_ids` ∪ `stale|failed|pending` в `scene_assets` ∪ FS-проба `getSceneFilesStatus` ∪ `hasRealAudio` — **Redis-состояния не читаются** (иммунитет к C0 over-marking, P1).
    - `build_id` пинится к `manifest.build_id` из `book.json` (fallback `'default'`); tombstone-книги пропускаются fail-closed (`getAllCancelled`); отключённые слои не получают PENDING (layerCfg из Redis, восстановленный C6); **сам C7 ничего не диспатчит** — только обычный scheduler.
    - **Safe materialization (follow-up gap 1):** PENDING-таргеты пишутся ТОЛЬКО через фасад — `setScenePending` для NEW/DIRTY/FAILED (валидные переходы) и двухшаг `markDirtyScene` (READY/PLACEHOLDER→DIRTY) → `setScenePending` (DIRTY→PENDING) для C0 over-mark. `unsafeRestoreAssetState` — только для READY-таргетов (restore-fact, guarded): `GENERATING`/`PENDING` current **никогда не трогается** recovery → downgrade выполняющейся генерации структурно невозможен (тест: GENERATING не downgrade). S2.4 whitelist-комментарий в `scene-state.js` обновлён.
    - **Dirty-intent реально влияет на predicate (follow-up gap 2):** раньше `dirtyScenes` собирался, но не консультировался — `dirty_unit_ids` и per-asset статусы теряли regeneration intent. Теперь: stale (version-mismatch ready) → PENDING; `scene_assets.status IN (stale,failed,pending)` по `asset_type` → PENDING даже при валидном leftover-файле; `dirty_unit_ids` → image PENDING (force-regen юнитов).
    - **Disabled-layer семантика доказана через реальный call graph (follow-up gap 3):** в `shouldScheduleAssets` `enabled = layerCfg.* !== false || isInFlight(state)` — для отключённого слоя с отсутствующим state: `false || isInFlight('new')` = **false** → никогда не диспатчится. Опасность была бы, если бы C7 записал PENDING для отключённого слоя (isInFlight сработал бы) — C7 пропускает disabled-стадии, доказано тестом через реальный `shouldScheduleAssets`.
    - Покрыто `backend/tests/worklist-rebuild.integration.test.js` (15 сценариев через реальный `reconcileCycle`/`rebuildWorkList`: half-gen, **идемпотентность C7**, fully-valid no-op, cancelled skip, disabled-layer, version-stale, GENERATING-not-downgraded, C0 self-heal, PLACEHOLDER→PENDING, dirty_unit_ids force-regen, failed/pending re-dispatch, disabled-layer no-NEW→dispatch через реальный call graph, disabled-layer с PENDING → dispatch proof, **TOCTOU: конкурентный tick между guard-read и записью не перезаписывается** (fresh-state re-read + отступ; in-flight GENERATING выживает)). **Full suite: 1117 passing, 0 failing.**
    - **Intentional семантика:** строка фазы `worklist_rebuild:N` считает **сцены** (scenes added), не книги. `failed`-строка image/video с существующим файлом → READY (без ре-диспатча) — это осознанный выбор: сбой мог произойти уже после записи артефакта; валидность файла определяется FS-пробой `getSceneFilesStatus` (version-stale gate применяется к `ready`-строкам). `hasRealAudio` probe failure логируется warn'ом и fail-closed трактует аудио как невалидное (PENDING), не глотая ошибку молча.
    - **Isolation note:** `purgeMockedModules` + re-bind модулей в `before()` — order-dependent workaround против pollution от happy-path.test.js / scope-slide.test.js (require.cache для runtime-config). Безопасен, пока файл остаётся последним по алфавиту ('w'); при добавлении файлов после 'w' workaround требует пересмотра (durable fix — restore cache в самих happy-path/scope-slide).
    - Дизайн-док обновлён (§12 Implementation status).

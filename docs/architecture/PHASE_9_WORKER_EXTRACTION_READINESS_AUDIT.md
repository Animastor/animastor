...
## Приложение B: исполнение Phase 9D (2026-09-06)

Порядок §7 выполнен: 9A (contract freeze) → 9B (dependency isolation) →
9C (`@animastor/contracts`) → **9D (physical extraction — ВЫПОЛНЕНО)**.

Итоги 9D (детали и прогоны: PHASE_9D_WORKER_PHYSICAL_EXTRACTION_AUDIT.md):

- Blocker B2 решён вариантом B: бандл несёт GENERATED byte-copy
  (`worker/worker/job-protocol-v2.cjs`) canonical contracts с
  parity/hash/version guards; inline-литералы worker.cjs удалены — вторая
  реализация протокола в worker устранена.
- Package boundary формализован: `animastor-worker@2.1.0` (private,
  zero-deps, files allowlist, engines, README, package-owned zero-dep
  tests в `worker/tests/`).
- Физический путь `worker/worker/` сохранён намеренно (4 deployment-канала
  + ~20 пинов ссылаются на него; перемещение = риск без runtime-выгоды).
- Standalone-запуск доказан: npm pack → чистая директория → `node worker.cjs`
  (fail-closed без токена; полный старт с токеном).
- Оставшиеся blockers 9E: E1 npm publish/auth, E2 Node floor
  (start-worker.sh 18 vs worker 20+), E3 hub contracts (B1), E4 version
  bump policy.

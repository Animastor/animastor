...
| O-8 | **physical move** (only after O-2..O-5 + O-7 guards green — the former O-7 HubCancelPort slot is now O-7b): `packages/animastor-orchestration` per §32.13 tree; `git mv`; consumer re-point via package root; delete dead `runtime-persistence` + `initializeRuntime` in a separate cleanup commit | — | — | O-G full suite + S5/S6/G7 regression | package loads standalone with bound ports; full-suite parity vs worktree baseline |

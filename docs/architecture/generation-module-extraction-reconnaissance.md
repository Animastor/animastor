...
| **O-P10 HubCancelPort** | `clearHubDispatches(dispatchIds, opts)` | orchestrator cancel leg (orchestrator.js:515), reconciliation STALE_LEASE fix | backend host (gpu-dispatcher/fetch) | Hub HTTP must not enter the package; today it reaches in through dispatch-engine's own export — **DONE at O-9 (§32.26): runtime/hub-cancel-port + storage/hub-cancel-adapter** |
...
|---|---|---|---|---|---|
| `book` facade (`@animastor/vbook-runtime` shims) — orchestration ×2, runtime ×3 | direct require | YES — scene content is the input data of orchestration | YES (package must not know the Book Model) | O-P2 | **DONE at O-3** — the 5 require sites were replaced by runtime/scene-data-port (§32.20); the VBook binding moved to the host adapter storage/scene-data-adapter |
...
| O-3 | **O-P2 SceneDataPort** + scene-asset-registry cleanup | the 5 `book` require sites; `services/scene-asset-registry` lazy orchestrator deps → composition root | orchestration→VBook facade, services→orchestration | O-G: zero `../book` requires; inbound services→orchestration = 0 | **DONE (see §32.20): both tiers consume scene content ONLY via runtime/scene-data-port — zero book requires; scene-asset-registry re-pointed to the canonical state-layer owner; inbound services→orchestration = 0** |
| O-4 | **O-P7 PlaceholderAudioPort** | scene-callbacks, scene-restoration, scene-window, reconciliation-engine; impl in backend/storage adapter | orchestration/runtime→services placeholder-audio host service | O-G: zero `services/placeholder-audio` requires in both tiers | **DONE (see §32.21): both tiers consume placeholder-audio ONLY via runtime/placeholder-audio-port — zero host-service requires** |
| O-4b | **O-P3 MediaFsmPort** (deferred from §32.14 planned O-4) | scene-orchestrator, orchestrator, reconciliation, scene-window | orchestration/runtime→services audio-orchestrator/video-orchestrator FSM adapters | O-G: zero `services/audio-orchestrator|video-orchestrator` requires | media FSM knowledge behind port (11 edges, ~25 ops + PHASES constants — deferred per minimal-seam mandate) |
| O-5 | **O-P4 ProgressEventsPort** (the §32.14 O-P4 half of planned O-5 — O-P5 split out per the one-seam mandate; see §32.22) | scene-callbacks (progress-pubsub ×5), runtime-scheduler (task-registry reads ×3), scene-window (hasActiveTasks gate); impl in backend/storage adapter | orchestration/runtime→services progress-pubsub + generation-progress | O-G: zero `services/progress-pubsub`/`services/generation-progress` requires in both tiers | **DONE (see §32.22): both tiers consume progress ONLY via runtime/progress-events-port — zero progress host-service requires; O-P5 OrchestrationConfigPort remains its own next seam (config/env out)** |
| O-6 | audit step — next host-boundary seam selection | (measured: MediaFsm ×27 members / 2 FSMs, Config ×~15 heterogeneous members / 6 files) | none closed — no candidate satisfies the seam priorities without a prohibition | existing O-G + O-1..O-5 suites | **BLOCKED (see §32.23): no safe single seam; both named candidates fail the minimal-contract mandate (defer-split options recorded); production untouched** |
| O-7 | **AudioFsmPort** (the per-media-FSM split of O-P3 authorized at §32.23 — audio half only, one seam) | scene-orchestrator (dispatch: 9 ops + PHASES), orchestrator (failStage sync: setFailed), scene-window (startScene: initPlaceholderReady), reconciliation-engine (stall watchdog + invariant checks + startup recovery: scanAllStates/failWaitingScene/completeChunk/getState/setState/setDone/setFailed/deleteState + PHASES); impl in backend/storage adapter | orchestration/runtime→services audio-orchestrator | O-G: zero `services/audio-orchestrator` requires in both tiers; frozen 12-op + PHASES contract; video FSM stays host-side | **DONE (see §32.24): both tiers drive the audio FSM ONLY via runtime/audio-fsm-port — zero audio-orchestrator host-service requires; optional-load reconciliation semantics preserved; video-orchestrator untouched (its own future seam)** |
...
| O-7 | **AudioFsmPort** (the per-media-FSM split of O-P3 authorized at §32.23 — audio half only, one seam) | scene-orchestrator (dispatch: 9 ops + PHASES), orchestrator (failStage sync: setFailed), scene-window (startScene: initPlaceholderReady), reconciliation-engine (stall watchdog + invariant checks + startup recovery: scanAllStates/failWaitingScene/completeChunk/getState/setState/setDone/setFailed/deleteState + PHASES); impl in backend/storage adapter | orchestration/runtime→services audio-orchestrator | O-G: zero `services/audio-orchestrator` requires in both tiers; frozen 12-op + PHASES contract; video FSM stays host-side | **DONE (see §32.24): both tiers drive the audio FSM ONLY via runtime/audio-fsm-port — zero audio-orchestrator host-service requires; optional-load reconciliation semantics preserved; video-orchestrator untouched (its own future seam)** |
| O-7b | **O-P10 HubCancelPort** | orchestrator.js:515, reconciliation STALE_LEASE leg | hub HTTP out of domain code | O-G: no fetch/http in moved set | **DONE (see §32.26): both tiers purge cancelled hub dispatches ONLY via runtime/hub-cancel-port — the last hub-HTTP edge in domain code is closed (1-op seam)** |
| O-8 | **VideoFsmPort** (the per-media-FSM split of O-P3 second half — §32.23 unblock path (a); never merged with the audio FSM) | scene-orchestrator (dispatch: getState/initState/markGroupDone/isGroupFileValid/groupFilePath/completeGroup/setWaitingChunks), orchestrator (failStage sync: setFailed), reconciliation-engine (stall watchdog + invariant checks + startup recovery: scanAllStates/PHASES/failWaitingScene/completeGroup/groupSuffixes/isGroupFileValid/getState/setDone/setFailed/deleteState); impl in backend/storage adapter | orchestration/runtime→services video-orchestrator | O-G: zero `services/video-orchestrator` requires in both tiers; frozen 13-op + PHASES contract; audio FSM never merged | **DONE (see §32.25): both tiers drive the video FSM ONLY via runtime/video-fsm-port — zero video-orchestrator host-service requires; optional-load reconciliation semantics preserved; the per-media-FSM split of O-P3 is COMPLETE (O-7 audio + O-8 video)** |
| O-9 | **physical move** (only after O-2..O-5 + O-7 + O-8 + O-9(b) guards green — the former O-7 HubCancelPort slot closed at O-9): `packages/animastor-orchestration` per §32.13 tree; `git mv`; consumer re-point via package root; delete dead `runtime-persistence` + `initializeRuntime` in a separate cleanup commit | — | — | O-G full suite + S5/S6/G7 regression | package loads standalone with bound ports; full-suite parity vs worktree baseline |
| O-10 | publish ceremony (deferred, mirror of S-9) | — | — | npm pack/clean-install audit | public release when needed |
...
**First concrete seam: O-1 — DONE** (§32.18). **The substantive first port seam O-2 (PersistencePort) — DONE** (§32.19) — it removed the largest single class of host edges (PG ×11 tier sites → 0 direct sites). **O-3 (SceneDataPort) — DONE** (§32.20) — it removed the VBook facade edges (×5 `../book` sites → 0) and the last inbound non-host consumer (services→orchestration = 0). **O-4 (PlaceholderAudioPort) — DONE** (§32.21) — it removed the placeholder-audio host-service edges (×4 tier sites → 0 direct sites), the third-largest host dependency class.

**Blockers for extraction (NOT for seam work):** O-P1..O-P10 do not exist yet (O-P1 done as the tier-owned PersistencePort at O-2, O-P2 as SceneDataPort at O-3 — both move into the package at O-8); media planning/validation knowledge in scene-orchestrator/scene-callbacks; OUTPUT_DIR env reads ×3; dead `runtime-persistence`/`initializeRuntime` and orphaned `retention-manager` should be removed before any move; consumer re-pointing (routes/services deep paths → package root) pending. As of O-3 the two largest host-edge classes are gone: direct PG require sites 11→0 (§32.19) and direct book-facade sites 5→0 (§32.20).
...
### 32.20 O-3 execution result — SceneDataPort extraction (DONE)

**Date:** 2026-09-11 · **Commit:** "arch(orchestration): introduce scene data port" · **Behavior-neutral seam commit. No physical move (O-8 untouched).**

**Reconnaissance corrections (measured at O-3 time, not doc-guessed):** the §32.11 "×5 `../book` sites" figure was CONFIRMED exact — `orchestration/scene-orchestrator.js:13` (top-level), `orchestration/scene-callbacks.js:29` (top-level), `runtime/scene-window.js:22` (top-level), `runtime/runtime-scheduler.js:439` (lazy), `runtime/reconciliation-engine.js:98,2126` (lazy ×2; 2,126 was the C-phase book cache-miss leg added with O-2). The actual VBook operations used by the two tiers are EXACTLY THREE (not the wider surface an earlier draft implied): `loadBook`, `findSceneRuntimeData`, `collectScenes`. The `services/scene-asset-registry → orchestration` edge is TWO lazy requires of `orchestrator.markDirtyScene` — which is itself a pure re-export of `state/scene-state-ops.markDirtyScene` (host-owned state layer), so the correct fix is a re-point to the canonical owner, NOT a new port or a hidden reverse channel.

**Port contract (`backend/src/runtime/scene-data-port.js`, tier-owned, zero-require):**

| Op | Signature shape | Consumers (measured) |
|---|---|---|
| `loadBook` | `(bookId) → loadedBook \| null` | scene-orchestrator dispatch fallback ×3, scene-callbacks auto-slide, scene-window window-complete + trySlide, runtime-scheduler tick cache-miss leg, reconciliation resolveBookBuildId + rebuildWorkList |
| `findSceneRuntimeData` | `(loadedBook, chapterId, sceneId) → scene \| null` | scene-orchestrator per-stage dispatch legs ×3 |
| `collectScenes` | `(loadedBook) → scenes[]` | scene-window isWindowComplete + trySlide, reconciliation rebuildWorkList |

`setSceneDataPort` fail-fasts on any missing op; call-time `sceneDataOp(op)` fail-fasts when unwired. The `loadedBook` argument stays the same opaque value the tiers already pass around. No SQL, no VBook internals, no Express, no future CRUD. Deliberately NOT in the port: the rest of the VBook facade (save/delete/import/sessions), lazy-book deep modules, book-model internals. No op overlaps PersistencePort (O-2) — the two contracts are disjoint.

**Host adapter (`backend/src/storage/scene-data-adapter.js`, host-owned):** resolves `backend/src/book` (the VBook shim) at CALL time inside each method — the established O-2 lazy-resolution discipline — so the require.cache-based VBook stubbing in orchestration-stabilization/scene-window/reconciliation test harnesses stays byte-identical effective. The adapter is the ONLY new raw-book consumer (one host edge replaces five tier edges; P7-T4 baseline updated consciously).

**Consumers migrated (5 files, every require site):** orchestration/scene-orchestrator (top-level), orchestration/scene-callbacks (top-level), runtime/scene-window (top-level), runtime/runtime-scheduler (lazy), runtime/reconciliation-engine (lazy ×2). Both tiers now bind ONLY `runtime/scene-data-port`.

**services→orchestration closure:** `services/scene-asset-registry.js` lazy requires of `orchestration/orchestrator` (for `markDirtyScene`) re-pointed to `state/scene-state-ops` — the canonical implementation owner the orchestrator was re-exporting. One-directional (services→state, which already existed elsewhere in services). No hidden reverse dependency introduced.

**Composition root:** `backend.cjs` calls `setSceneDataPort(makeSceneDataAdapter())` BEFORE the first top-level tier require (adjacent to the O-2 persistence wiring); `tests/generation-test-bindings.cjs` mirrors production wiring for the mocharc require chain.

**Guards — NEW `tests/architecture/o3-scene-data-port.test.js` (10):** O3-G1 zero `../book` requires in both tiers (static + dynamic-proximity); O3-G2 zero lazy/dynamic VBook requires (computed-require scan, mirrors Phase-5 T9 discipline); O3-G3 port is zero-require and host-knowledge-free; O3-G4 op-set minimality (every tier `sceneDataOp('…')` declared in OPS); O3-G5 no SQL/PG/VBook-implementation leakage through the port; O3-G6 scene-asset-registry has zero orchestration requires (static + dynamic); O3-G7 composition-root wiring precedes the first top-level tier require; O3-G8 O-1 shim stays deleted; O3-G9 O-2 PersistencePort boundary intact; O3-G10 runtime→orchestration stays 0.

**Existing guards updated (consciously, per their own conventions):** `phase7-extraction-readiness.test.js` P7-T4 RAW_BOOK_BASELINE −5 tier entries / +1 host adapter entry with O-3 notes; `phase5-runtime-result.test.js` T9 — the port comment was reworded to keep runtime/** free of residual computed-require lookalikes (the guard is comment-sensitive by design).

**Dependency/SCC audit (post-change, full-tree Tarjan, static + lazy — `scripts/o3-scc-audit.cjs`):** nodes 241, lazy edges 155. runtime→orchestration: 0. services→orchestration: 0. orchestration→book: 0. runtime→book: 0. orchestration→storage: 0. runtime→storage: 3 (pinned gpu-dispatcher host adapter, unchanged O-2 baseline). SCCs >1 node: 1 (pre-existing `services/workspace-ai-provider ↔ services/system-ai`, host side, unchanged). No new reverse dependency, no new SCC.

**Tests:** full backend suite `3308 passing, 13 failing` vs pristine-parent (88cd791e) worktree `3289 passing, 22 failing`; the failure-set on this branch is a strict SUBSET of the pristine baseline — 0 O-3 regressions (the 9 baseline-only failures are flaky environment/PG-timeout suites; the branch run caught them green). Named regression targets all green: runtime-orchestration-recon, s5-runtime-orchestration-cycle, dependency guardrails (R-series), orchestration-stabilization (87), reconciliation-engine + scene-asset-registry + counter-reconciliation (78), O-2 guards (6/6), Phase-5/Phase-7 suites, sql-boundary + PG-1..PG-5 (unchanged baselines).

**Not done (out of scope, per mandate):** no physical move; O-P3 MediaFsmPort is the NEXT seam candidate (§32.14 O-4b, 11 edges / ~25 ops + PHASES constants — deferred per minimal-seam mandate); Generation API, HTTP API, VBook behavior, SQL semantics untouched.

### 32.21 O-4 execution result — PlaceholderAudioPort extraction (DONE)

**Date:** 2026-09-12 · **Commit:** "arch(orchestration): introduce placeholder audio port" · **Behavior-neutral seam commit. No physical move (O-8 untouched).**

**Reconnaissance corrections (measured at O-4 time, not doc-guessed):** the §32.14 "×4 tier placeholder-audio require sites" figure was CONFIRMED exact — `orchestration/scene-callbacks.js:34` (top-level), `orchestration/scene-restoration.js:11` (top-level), `runtime/scene-window.js:41` (top-level), `runtime/reconciliation-engine.js:2137` (lazy inside rebuildWorkList). The actual operations used by the two tiers are EXACTLY THREE: `hasRealAudio` (5 call sites across 3 files), `ensurePlaceholderAudio` (1 call site), `replacePlaceholderWithRealAudio` (1 call site). The host service (`services/placeholder-audio.js`) is heavy host infrastructure: ffmpeg child-process (`generateSilentAudio`), fs path composition, PG scene_assets/iu_repo reads and writes, `config.OUTPUT_DIR`, and book facade fallback (`../book`, `../book/lazy-book` — pinned in P7-T4 RAW_BOOK_BASELINE). This is O-P7 from §32.7.

**Port contract (`backend/src/runtime/placeholder-audio-port.js`, tier-owned, zero-require):**

| Op | Signature shape | Consumers (measured) |
|---|---|---|
| `hasRealAudio` | `(bookId, chapterId, sceneId, buildId) → Promise<boolean>` | scene-window ×3 call sites (:204, :317, :590), scene-restoration restore probe (:29), reconciliation rebuildWorkList (:2222) |
| `ensurePlaceholderAudio` | `(buildId, bookId, chapterId, sceneId) → Promise<{created, path, durationSec, reason}>` | scene-window addToWindow (:763) |
| `replacePlaceholderWithRealAudio` | `(bookId, chapterId, sceneId, buildId, realAudioPath, realDuration) → Promise<{replaced, reason}>` | scene-callbacks handleAudioCompleted (:137) |

`setPlaceholderAudioPort` fail-fasts on any missing op; call-time `placeholderAudioOp(op)` fail-fasts when unwired. No SQL, no ffmpeg, no fs, no OUTPUT_DIR, no host-service internals. No op overlaps PersistencePort (O-2) or SceneDataPort (O-3) — the three contracts are disjoint. Deliberately NOT in the port: `estimateSpeechDurationSec` (pure text heuristic with canonical owner `utils/speech-estimation` — consumed by VBook agent pipeline, not by tiers), `ensureAllPlaceholderAudio`, `getScenesNeedingPlaceholder`, `markPlaceholderStale`, `recoverMissingPlaceholders`, and any CRUD "for later".

**Host adapter (`backend/src/storage/placeholder-audio-adapter.js`, host-owned):** resolves `services/placeholder-audio` at CALL time inside each method — the established O-2/O-3 lazy-resolution discipline — so the require.cache-based placeholder-audio stubbing in happy-path D.3 (scene-window) and Section-4 (scene-callbacks) test harnesses stays byte-identical effective. The adapter is the ONLY new placeholder-audio consumer (one host edge replaces four tier edges).

**Consumers migrated (4 files, every require site):** orchestration/scene-callbacks (top-level → port, 1 call site), orchestration/scene-restoration (top-level → port, 1 call site), runtime/scene-window (top-level → port, 4 call sites), runtime/reconciliation-engine (lazy → port, 1 call site inside rebuildWorkList). Both tiers now bind ONLY `runtime/placeholder-audio-port`.

**Composition root:** `backend.cjs` calls `setPlaceholderAudioPort(...)` AFTER O-3 SceneDataPort wiring and BEFORE the first top-level tier require; `tests/generation-test-bindings.cjs` mirrors production wiring for the mocharc require chain. Wiring order: O-2 → O-3 → O-4.

**Guards — NEW `tests/architecture/o4-placeholder-audio-port.test.js` (10):** O4-G1 zero `../services/placeholder-audio` requires in both tiers (static scan); O4-G2 port is zero-require and host-knowledge-free (no ffmpeg, no SQL, no tables, no fs); O4-G3 op-set minimality (every tier `placeholderAudioOp('…')` declared in OPS, every declared op has a live consumer); O4-G4 adapter exports frozen to exactly three ops; O4-G5 composition-root wiring precedes the first top-level tier require; O4-G6 no lazy/dynamic bypass around the port (dynamic-require proximity scan); O4-G7 O-1 shim stays deleted; O4-G8 O-2 PersistencePort boundary intact; O4-G9 O-3 SceneDataPort boundary intact; O4-G10 runtime→orchestration stays 0.

**Existing guards updated (consciously, per their own conventions):** `phase7-extraction-readiness.test.js` P7-T4 RAW_BOOK_BASELINE unchanged (host service keeps its `../book` edges); `phase5-runtime-result.test.js` T9 unchanged (no computed-require lookalikes introduced).

**Dependency/SCC audit (post-change, full-tree Tarjan, static + lazy — `scripts/o3-scc-audit.cjs`):** nodes 243, lazy edges 155. runtime→orchestration: 0. services→orchestration: 0. orchestration→book: 0. runtime→book: 0. orchestration→storage: 0. runtime→storage: 3 (pinned gpu-dispatcher host adapter, unchanged O-2 baseline). runtime→services/placeholder-audio: 0. orchestration→services/placeholder-audio: 0. SCCs >1 node: 1 (pre-existing `services/workspace-ai-provider ↔ services/system-ai`, host side, unchanged). No new reverse dependency, no new SCC.

**Tests:** full backend suite `3318 passing, 13 failing` vs pristine-parent baseline `3308 passing, 13 failing` (the 10-test delta is the new O-4 guard suite). All 13 failures are identical pre-existing environment-dependent baseline failures — 0 O-4 regressions. Named regression targets all green: O-4 guards (10/10), O-2 guards (6/6), O-3 guards (10/10), O-G series (10/10), s5-runtime-orchestration-cycle (7/7), dependency guardrails (R-series), orchestration-stabilization, reconciliation-engine, counter-reconciliation, fail-stage, dispatch-meta-lease-lifecycle, scope-slide, scene-split, visuals-duration, video-action-polish, full `test:arch` (889).

**Not done (out of scope, per mandate):** no physical move; O-P3 MediaFsmPort is the NEXT seam candidate (§32.14 O-4b — 11 tier edges to services/audio-orchestrator|video-orchestrator, ~25 ops + PHASES constants — deferred per minimal-seam mandate); Generation API, HTTP API, VBook behavior, SQL semantics untouched.

### 32.22 O-5 execution result — ProgressEventsPort extraction (DONE)

**Date:** 2026-09-12 · **Commit:** "arch(orchestration): introduce progress events port" · **Behavior-neutral seam commit. No physical move (O-8 untouched).**

**Reconnaissance (measured at O-5 time on b15cdc39, full static+lazy graph, `scripts/o3-scc-audit.cjs`):** nodes 243, lazy edges 155, runtime→orchestration 0, orchestration→book 0, runtime→book 0, orchestration→storage 0, runtime→storage 3 (pinned gpu-dispatcher host transport), SCCs >1 node: 1 (pre-existing host-side `services/workspace-ai-provider ↔ services/system-ai`). Closed O-1/O-2/O-3/O-4 boundaries re-verified green. Remaining tier→host edges by contour: `services/audio-orchestrator|video-orchestrator` ×11 (O-P3, deferred §32.14 O-4b — ~25 ops + PHASES, too wide for one seam), `config/runtime-config` ×6 files (O-P5 — GPU_TIMEOUT/LEASE_TTL/WORKER_HEARTBEAT/QUOTAS/REDIS keys/OUTPUT_DIR/HUB_URL/API key: not one small stable contract, god-interface risk), `state/**` ×5 files (S-5 seam boundary, owned), `audio/image/video` media modules (O-P6), fs/path/process.env OUTPUT_DIR sites (folds into O-P5), gpu-dispatcher PG repos ×3 (pinned host transport until its own step). §32.14 planned O-5 as "O-P5 + O-P4" — but the minimal-seam mandate forbids two independent seams in one step; **O-P4 ProgressEventsPort was selected** (criteria A/B/C all true): two narrow host facades already exist (services/progress-pubsub.cjs — SSE Redis pub/sub channel `animastor:progress:*`; services/generation-progress.js — Redis task-registry hash `animastor:generation-progress:*` over the pure Generation core), 3 tier consumers across BOTH tiers (scene-callbacks, runtime-scheduler, scene-window), exactly 4 measured operations, host implementations byte-stable, no god interface (the task-registry's 10-op CRUD surface stays routes-side), extraction removes 4 tier→services edges and kills the last services knowledge inside scene-callbacks' completion path. O-P5 (config/env) stays the next candidate — noted in §32.14 as amended.

**Port contract (`backend/src/runtime/progress-events-port.js`, tier-owned, zero-require):**

| Op | Signature shape | Consumers (measured) |
|---|---|---|
| `publishProgress` | `(redis, bookId, event) → Promise<void>` (best-effort, never throws) | scene-callbacks ×5 call sites (audio :262, video :436, generation_complete ×3 :457/:493/:521) |
| `getSceneTaskState` | `(redis, bookId, chapterId, sceneId) → Promise<{managed, activeTypes}>` | runtime-scheduler shouldScheduleAssets (:298) + markVersionStaleDirty (:264) |
| `hasActiveTasks` | `(redis, bookId) → Promise<boolean>` | scene-window trySlideWindowOnComplete task_managed gate (:450) |
| `reconcileCompletedTasks` | `(redis, bookId, getAssetStates) → Promise<task[]>` (callback arg passes through exactly as pre-O-5) | runtime-scheduler tick pre-pass (:398) |

`setProgressEventsPort` fail-fasts on any missing op; call-time `progressEventsOp(op)` fail-fasts when unwired. No Redis key strings, no pub/sub transport, no TTLs, no SSE knowledge, no task CRUD. Deliberately NOT in the port: createTasks/listTasks/getTask/updateTask/markCompleted/markCancelled/removeTask/clear/getActiveTasksByType (routes-side HTTP contour owns task lifecycle — import-routes, generation-routes, progress-panel keep their direct host requires) and any CRUD "for later". No op overlaps O-2/O-3/O-4 — the four contracts are disjoint.

**Host adapter (`backend/src/storage/progress-events-adapter.js`, host-owned):** resolves BOTH host services at CALL time inside each method (`() => require('../services/progress-pubsub.cjs')`, `() => require('../services/generation-progress')`) — the established O-2/O-3/O-4 lazy-resolution discipline — so the require.cache-based harness conventions keep working byte-identically (image-orphan-generating-repair P.genProgress purge+reload; scope-slide's real-service task-managed gate test; worklist-rebuild MOCKED_PATHS re-binding). The adapter is the ONLY new progress consumer (one host edge replaces three tier edges; each op delegates 1:1 — identical arguments, return shapes and error semantics, including publishProgress's never-throw catch).

**Consumers migrated (3 files, every require site):** orchestration/scene-callbacks (top-level → port, 5 call sites), runtime/runtime-scheduler (top-level → port, 3 call sites), runtime/scene-window (lazy call-site require → port, 1 call site — lazy resolution preserved). Both tiers now bind ONLY `runtime/progress-events-port`.

**Composition root:** `backend.cjs` calls `setProgressEventsPort(...)` AFTER O-4 PlaceholderAudioPort wiring and BEFORE the first top-level tier require; `tests/generation-test-bindings.cjs` mirrors production wiring for the mocharc require chain. Wiring order: O-2 → O-3 → O-4 → O-5.

**Guards — NEW `tests/architecture/o5-progress-events-port.test.js` (10):** O5-G1 zero `../services/progress-pubsub(.cjs)` and `../services/generation-progress` requires in both tiers (static scan); O5-G2 port is zero-require and host-knowledge-free (no Redis channel/key strings, no pub/sub/SSE transport, no client APIs); O5-G3 op-set minimality (every tier `progressEventsOp('…')` declared in OPS, every declared op has a live consumer, frozen four-op set); O5-G4 adapter exports frozen to exactly four ops + host services keep their public surfaces pinned; O5-G5 composition-root wiring precedes the first top-level tier require (both roots) + O-2→O-3→O-4→O-5 order stability; O5-G6 no lazy/dynamic progress bypass (dynamic-require proximity scan + adapter call-time resolution is the single channel); O5-G7 O-1 shim stays deleted; O5-G8 O-2/O-3/O-4 boundaries intact; O5-G9 runtime→orchestration stays 0; O5-G10 the seam smuggles no env/config (no process.env, no runtime-config in port/adapter — O-P5 stays a separate seam) and the O-G9 env baseline does not grow.

**Existing guards:** no updates required — phase6-editor-player baselines (routes contours, untouched), redis-registry (Redis families unchanged — the channel/hash bytes are identical), S4 (generation-progress adapter API frozen, untouched), P7-T4 (no book edges introduced). O-G9's orchestration env pin is UNCHANGED by O-5 (the 3 OUTPUT_DIR sites are scene-callbacks/scene-restoration fs concerns owned by the future O-P5 seam; the O-5 port deliberately does not touch env/config).

**Dependency/SCC audit (post-change, full-tree Tarjan, static + lazy — `scripts/o3-scc-audit.cjs`):** nodes 245 (+2: port + adapter), lazy edges 156. runtime→orchestration: 0. services→orchestration: 0. orchestration→book: 0. runtime→book: 0. orchestration→storage: 0. runtime→storage: 3 (pinned gpu-dispatcher host transport, unchanged). runtime→services/generation-progress: 0. runtime→services/progress-pubsub: 0. orchestration→services/progress-pubsub: 0. SCCs >1 node: 1 (pre-existing `services/workspace-ai-provider ↔ services/system-ai`, host side, unchanged). No new reverse dependency, no new SCC.

**Tests (named targets, this branch vs identical-command pristine worktree runs):** O-5 guards (10/10), O-2 guards (6/6), O-3 guards (10/10), O-4 guards (10/10), O-G series (10/10), s5-runtime-orchestration-cycle (7/7) — all green. Tier/host regression suites green: orchestration-stabilization + reconciliation-engine + scope-slide + generation-progress (93), happy-path + image-orphan-generating-repair + dispatch-meta-lease-lifecycle (116), worklist-rebuild + counter-reconciliation + fail-stage + scene-split (60), generation-routes + progress-panel + cancellation-recovery (12), execute-lifecycle + audio-orchestrator + video-orchestrator (79), gen-scope + runtime-timeouts + generation-provider-seam (51), full `test:arch` 899 (= 889 baseline + 10 new O-5 guards). Two pre-existing environment failures confirmed IDENTICAL on pristine-parent runs (worker-share-grants before-all 60s PG/auth timeout; bootstrap-cancel-continue "AI assistant is not available" — OPENROUTER_API_KEY absent in this environment) — 0 O-5 regressions. No GitHub status available for this run.

**Not done (out of scope, per mandate):** no physical move; O-P5 OrchestrationConfigPort (config/env/OUTPUT_DIR — closes the O-G9 env pin) and O-P3 MediaFsmPort (§32.14 O-4b) remain the next seam candidates, each as its own single seam step; Generation API, HTTP endpoints, VBook API, Redis/FSM semantics, SQL semantics untouched.

### 32.23 O-6 reconnaissance audit — next host-boundary seam (BLOCKED — no seam extracted)

**Date:** 2026-09-12 · **Baseline:** 521919d2 (branch c21.4-physically-extract-analysis-from-backend) · **Audit-only commit: NO production change.**

**Reconnaissance (measured at O-6 time on 521919d2, full static+lazy graph via `scripts/o3-scc-audit.cjs`):** nodes 245, lazy edges 156, runtime→orchestration 0, orchestration→book 0, runtime→book 0, orchestration→storage 0, runtime→storage 3 (pinned gpu-dispatcher host transport), SCCs >1 node: 1 (pre-existing host-side `services/workspace-ai-provider ↔ services/system-ai`). O-1..O-5 boundaries re-verified green BEFORE the audit decision (53 guards: O-2 6/6, O-3 10/10, O-4 10/10, O-5 10/10, O-G 10/10, S-5 7/7). Remaining tier→host edges re-measured file-by-file:

| Contour | Tier edges (files) | Members the tiers actually use |
|---|---|---|
| `services/audio-orchestrator` | 4 (scene-orchestrator, orchestrator, scene-window, reconciliation ×4 sites) | 13 ops + PHASES const object |
| `services/video-orchestrator` | 3 (scene-orchestrator, orchestrator, reconciliation ×4 sites) | 14 ops + PHASES const object |
| `config/runtime-config` | 6 (dispatch-engine, gpu-dispatcher, worker-health, scene-window, reconciliation, orchestrator) | ~15 heterogeneous members (TIMEOUTS.{AUDIO_CHUNK_STALL_MS,VIDEO_CHUNK_STALL_MS,IU_IN_FLIGHT_TTL_S,FORCE_DISPATCH_TTL_S}, OUTPUT_DIR ×6 sites, HUB_URL, GPU_HUB_API_KEY, REDIS.CLEANUP_LOCK, WORKER_HEARTBEAT_{KEY,TYPE_PATTERN,TTL,TYPES}, WINDOW_SIZE, shareFeaturesEnabled(), GPU_TIMEOUT_MS) + 3 raw `process.env.OUTPUT_DIR` sites (the O-G9 pin) |
| media modules (O-P6) | 6 files (audio ×3, image ×2, video ×2, music-metadata) | segments/chunks math, duration probing, canonical-PNG resolution — media implementation knowledge, not a stable contract |
| `state/**` | 5 files | S-5 seam boundary — owned, deliberately NOT a port |
| gpu-dispatcher PG repos | 3 (inside gpu-dispatcher itself) | pinned host transport until its own step (§32.19) |
| `services/layer-config`, `services/gen-scope` | 2 + 1 | §32.7 folds them into O-P5/O-P2 as accessors, not standalone ports |

**Seam evaluation against the priority criteria (A narrow host facade / B real consumers / C host-side implementation exists / D small stable contract / E substantial coupling cut / F 1:1 delegation):**

- **O-P3 MediaFsmPort — NOT SAFE now.** Audio tier usage alone spans 13 ops (getState, setState, deleteState, initPlaceholderReady, setGenerating, setWaitingChunks, setMerging, setDone, setFailed, completeChunk, failWaitingScene, scanAllStates, PHASES) and video spans 14 (initState, getState, markGroupDone, groupSuffixes, groupFilePath, isGroupFileValid, completeGroup, setWaitingChunks, setDone, setFailed, failWaitingScene, scanAllStates, deleteState, PHASES) — 27 members across TWO independent Redis state machines (`animastor:audio-orch:*` / `animastor:video-orch:*`). Fails criterion D twice over: 20+ heterogeneous operations, and the mandate explicitly forbids merging both FSMs into one port while the one-seam-per-step rule forbids extracting them as two. The try/catch-optional-load discipline around the reconciliation requires (missing-module → `return 0`) also means a fail-fast port would CHANGE error semantics (criterion: no semantics change). Defer until it can be split per media FSM (audio seam step, then video seam step), mirroring how O-4b was already deferred twice for exactly this reason.
- **O-P5 OrchestrationConfigPort — NOT SAFE now.** ~15 heterogeneous members across 6 tier files: scalar TTLs, a key-grammar table (REDIS), key-BUILDER functions (WORKER_HEARTBEAT_KEY/TYPE_PATTERN), a kill-switch function read lazily at call time (shareFeaturesEnabled — env-dependent semantics a snapshot port would freeze), OUTPUT_DIR path roots and HUB credentials. That is a god-interface by the mandate's own definition (this is the §32.22 recon note: "not one small stable contract, god-interface risk" — re-confirmed at O-6 with the exact member list). It also entangles the O-G9 env-pin closure (3 raw process.env sites) — a mixed config+env+fs-roots surface that the mandate forbids merging into one boundary. A defensible future shape is per-concern splits (e.g. a narrow WorkerHeartbeatKeysPort first, then a TimeoutsPort, then env/fs), but each of those is its own future seam step, not this one.
- **O-P6 MediaPlanningPort — NOT NOW.** Real consumers exist (scene-orchestrator, scene-callbacks STAGE_HANDLERS, reconciliation repair legs, scene-window audio-service) but the ops are media-implementation knowledge (buildSegments math, music-metadata probing, canonical-PNG resolution, chunk-existence scans) with no existing narrow host facade — extraction would mean porting media logic, which the mandate forbids ("no business-logic moves").
- **O-P10 HubCancelPort — NOT NOW (mandate-scoped out).** `clearHubDispatches` is a genuinely narrow op (1 function, 2 tier call sites: orchestrator cancel leg, reconciliation STALE_LEASE) BUT the implementation lives inside runtime/dispatch-engine itself — the §32.14 plan assigns hub-HTTP extraction to step O-7, and moving it now would be an out-of-sequence re-plan; it is also not one of the two candidates this audit was asked to weigh.
- **layer-config / gen-scope — NOT standalone ports.** §32.7 already folds them into O-P5/O-P2 as accessors; gen-scope has exactly ONE tier file consumer (scene-window) — criterion B (multiple real consumers) fails.

**Decision: BLOCKED.** No single remaining host boundary satisfies the seam priorities without violating a prohibition. Per the mandate (§8): production untouched, only this reconnaissance result is recorded; status O-6 = BLOCKED (audit-only; no port, no adapter, no consumer changes, no new guards — the O-1..O-5 guard suites remain the boundary net).

**Re-verified at close:** all 53 boundary guards green (O-2 6/6, O-3 10/10, O-4 10/10, O-5 10/10, O-G 10/10, S-5 7/7); graph/SCC unchanged (runtime→orchestration 0, no reverse dependency, no new SCC, tier→host edges identical to the §32.22 inventory). No production files changed in this step.

**Unblock path (recorded for the next task):** either (a) authorize a per-media-FSM split of O-P3 (AudioFsmPort as one seam step, VideoFsmPort as the next — both ≤14 ops, satisfying D), or (b) authorize a per-concern split of O-P5 (starting with the narrowest slice, e.g. WorkerHeartbeatKeys or Timeouts), each as its own single-seam step per the one-seam mandate. **Path (a) is now COMPLETE: AudioFsmPort DONE at O-7 (§32.24), VideoFsmPort DONE at O-8 (§32.25).**

### 32.24 O-7 — AudioFsmPort extraction seam (DONE)

**Date:** 2026-09-12 · **Baseline:** 5dcdc9d4 (branch c21.4-physically-extract-analysis-from-backend) · **Single-seam step, behavior-neutral.**

**Mandate:** the §32.23 unblock path (a) — AudioFsmPort as ONE seam step (the audio half of the deferred O-P3 MediaFsmPort), never merged with the video FSM. The O-6 rejection of the combined 27-member MediaFsmPort surface stands; this step takes only the audio half, measured independently.

**Reconnaissance (re-measured at O-7 on 5dcdc9d4, not trusted from docs):** the full static+lazy graph via `scripts/o3-scc-audit.cjs` confirmed the §32.23 baseline (245 nodes, 156 lazy edges, runtime→orchestration 0, orchestration→storage 0, runtime→storage 3 pinned gpu-dispatcher host transports, 1 pre-existing host-side SCC `services/workspace-ai-provider ↔ services/system-ai`). Tier→audio-FSM edges measured file-by-file, all lazy `require` call sites:

| Tier consumer | Call sites | Operations actually used |
|---|---|---|
| `orchestration/scene-orchestrator.js` (executeAudioDispatch) | 1 lazy require (line 131) | getState, PHASES, setGenerating, initPlaceholderReady, deleteState, completeChunk, setWaitingChunks, setMerging, setDone |
| `orchestration/orchestrator.js` (failStage audio leg) | 1 lazy require (line 303) | setFailed |
| `runtime/scene-window.js` (startScene placeholder init) | 1 lazy require (line 789) | initPlaceholderReady |
| `runtime/reconciliation-engine.js` (stall watchdog, invariant checks, startup recovery) | 3 lazy requires (394, 594, 1862) — two optional-load (`try/catch` → `return 0`), one mandatory | scanAllStates, PHASES, failWaitingScene, completeChunk, getState, setState, setDone, setFailed, deleteState |

**Union:** 12 ops (getState, setState, deleteState, initPlaceholderReady, setGenerating, setWaitingChunks, setMerging, setDone, setFailed, completeChunk, failWaitingScene, scanAllStates) + the PHASES phase-name constants the tiers compare `orchState.phase` against. Substantially narrower than the combined MediaFsmPort (27 members / 2 FSMs) and narrower than the O-6-measured "13 ops" audio half (transitionState/key/createState are host internals the tiers never touch; the O-6 count included them).

**Error semantics (measured, preserved exactly):**
- `checkStalledAudioScenes` and `recoverAudioOrchStates` (reconciliation-engine) wrap the require in `try { … } catch (_) { return 0; }` — a missing module is NOT a stalled/recoverable state. This optional-load contract is preserved by resolving the ADAPTER through the port inside the same try/catch (`require('./audio-fsm-port').audioFsm()` throws when the port is unwired; if the host service itself cannot be loaded, the adapter's lazy call-time resolver throws at the same point the pre-O-7 require did). No mechanical fail-fast was introduced where it would change behavior.
- `checkAudioOrchInvariants` was a MANDATORY load (no try/catch) pre-O-7 and stays mandatory post-O-7.
- `scene-orchestrator`/`orchestrator`/`scene-window` lazy requires were mandatory-at-call-time and stay mandatory-at-call-time through the fail-fast port.

**Seam (all criteria satisfied):** one independent FSM (single Redis state machine `animastor:audio-orch:*`, single phase machine NEW→PLACEHOLDER_READY→GENERATING→WAITING_CHUNKS→MERGING→DONE/FAILED) · contract ≪ MediaFsmPort · existing `services/audio-orchestrator` stays the host-side implementation unchanged · pure 1:1 delegation (identical args/returns/errors) · no business-logic move · no video FSM, no Redis key grammar, no transition-map internals in the port (phase NAMES are the FSM's public vocabulary; their STORAGE stays adapter-side).

**Implementation:**
- `backend/src/runtime/audio-fsm-port.js` — zero-require tier-owned contract: 12-op OPS list, PHASE_CONSTANTS vocabulary, `setAudioFsmPort` wiring gate (fail-fast on missing ops/PHASES), `audioFsm()`/`audioFsmOp()`/`audioFsmPhases()` call-time resolvers, `_resetAudioFsmPort` test hygiene.
- `backend/src/storage/audio-fsm-adapter.js` — the single bridge: lazy call-time `require('../services/audio-orchestrator')` (require.cache stubbing parity for the reconciliation/worklist harnesses), 12 × 1:1 delegations, `PHASES` passthrough getter.
- Consumers migrated (no parallel refactors): scene-orchestrator, orchestrator (audio leg only — video leg untouched), scene-window, reconciliation-engine ×3 sites. All 7 former `services/audio-orchestrator` tier edges deleted.
- Composition root `backend.cjs` wires `setAudioFsmPort(storage/audio-fsm-adapter)` after O-5, before the first top-level tier require; `tests/generation-test-bindings.cjs` mirrors it (order O-2 → O-3 → O-4 → O-5 → O-7).
- Guards: `backend/tests/architecture/o7-audio-fsm-port.test.js` O7-G1..G10 — zero tier host-service requires (static), zero-require port, exact frozen 12-op + PHASES contract with live-consumer minimality (both sanctioned consumption styles: `audioFsmOp('…')` and the whole-adapter `audioFsm()` handle with a bare-PHASES-alias provenance check), no FSM-implementation leakage (frozen adapter export set, no video FSM, no PREFIX/key/createState/transitionState), composition-root-before-consumer wiring + O-2→O-5→O-7 order, no dynamic bypass (tier proximity scan + adapter stays the single lazy channel), O-1/O-2/O-3/O-4/O-5 non-regression, runtime→orchestration = 0, adapter as the single new bridge with the frozen pre-O-7 host consumer set (routes/generation-routes.cjs, services/task-handler.cjs).

**Graph after (re-measured):** nodes 247 (+port, +adapter), lazy edges 155 (−7 tier→service, +6 tier→port, +1 adapter→service at lazy depth), runtime→orchestration 0, orchestration→storage 0, runtime→storage 3 (pinned), `runtime/orchestration → audio-orchestrator = 0` with the single host-side `storage/audio-fsm-adapter → services/audio-orchestrator` edge, SCCs >1 node: still exactly the 1 pre-existing host-side pair. VERDICT: PASS on all checks.

**Tests (this worktree vs identical-command baseline worktree at 5dcdc9d4):** O-7 guards 10/10 NEW; test:arch 909 (= 899 baseline + 10); O-2 6/6, O-3 10/10, O-4 10/10, O-5 10/10, O-G 10/10, S-5 7/7, dependency-guardrails green; tier/host suites green individually: audio-orchestrator (28), reconciliation-engine (55), orchestration-stabilization (11), fail-stage (6), scope-slide + worklist-rebuild + counter-reconciliation + scene-split + stage-dispatch-lifecycle + execute-lifecycle + generation-progress (122), happy-path + image-orphan-generating-repair + dispatch-meta-lease-lifecycle (116), video-orchestrator + gen-scope + runtime-timeouts (57), generation-routes + cancellation-recovery + progress-panel (12), generation-provider-seam green. Known pre-existing environment failures confirmed IDENTICAL on the pristine baseline worktree (worker-share-grants before-all 60s PG/auth timeout; the combined audio-orchestrator+reconciliation+stabilization+fail-stage file-order run has the same 5 fail-stage + 2 stabilization-hook failures on BOTH trees — require.cache interference between those two suites' orchestrator.js stubbing strategies, not an O-7 regression; the full 909+ suite's 13 failures are the documented worker-share-grants/ai-endpoint-sharing/fail-closed-worker-auth PG/OPENROUTER_API_KEY environment set, reproduced 1:1 at baseline). No GitHub status available for this run.

**Not done (out of scope, per mandate):** no physical move; VideoFsmPort (the mirrored seam for `services/video-orchestrator` — 3 tier files, 5 lazy sites, next single-seam candidate); O-P5 OrchestrationConfigPort; O-P6 MediaPlanningPort; O-P10 HubCancelPort (now slot O-7b); Generation API, VBook API, HTTP/Redis/FSM/SQL semantics untouched; `services/audio-orchestrator` public API unchanged (routes/task-handler consumers keep working).

### 32.25 O-8 — VideoFsmPort extraction seam (DONE)

**Date:** 2026-09-12 · **Baseline:** f059629f (branch c21.4-physically-extract-analysis-from-backend) · **Single-seam step, behavior-neutral.**

**Mandate:** the §32.23 unblock path (a) second half + the §32.24 recorded next candidate — VideoFsmPort as ONE seam step (the video half of the deferred O-P3 MediaFsmPort), never merged with the audio FSM. The O-6 rejection of the combined 27-member MediaFsmPort surface stands; this step takes only the video half, measured independently. Reconnaissance first; the seam was selected only after re-measuring the full graph and re-verifying O-1..O-7.

**Reconnaissance (re-measured at O-8 on f059629f, not trusted from docs):** the full static+lazy graph via `scripts/o3-scc-audit.cjs` confirmed the §32.24 baseline (247 nodes, 155 lazy edges, runtime→orchestration 0, orchestration→storage 0, runtime→storage 3 pinned gpu-dispatcher host transports, 1 pre-existing host-side SCC `services/workspace-ai-provider ↔ services/system-ai`). O-1..O-7 boundaries re-verified green BEFORE the seam decision (O-2 6/6, O-3 10/10, O-4 10/10, O-5 10/10, O-7 10/10, O-G 10/10, S-5 7/7; §32.24 §32.25 re-run: 919 total after the new guards, 909 before). Tier→video-FSM edges measured file-by-file, all lazy `require` call sites:

| Tier consumer | Call sites | Operations actually used |
|---|---|---|
| `orchestration/scene-orchestrator.js` (executeVideoDispatch) | 1 lazy require (line 386) | getState, initState, markGroupDone, isGroupFileValid, groupFilePath, completeGroup, setWaitingChunks |
| `orchestration/orchestrator.js` (failStage video leg) | 1 lazy require (line 309) | setFailed |
| `runtime/reconciliation-engine.js` (stall watchdog + invariant checks + startup recovery) | 3 lazy requires (504, 664, 1976) — two optional-load (`try/catch` → `return 0`), one mandatory | scanAllStates, PHASES, failWaitingScene, completeGroup, groupSuffixes, isGroupFileValid, getState, setDone, setFailed, deleteState |

**Union:** 13 ops (getState, initState, setWaitingChunks, setDone, setFailed, deleteState, markGroupDone, groupSuffixes, groupFilePath, isGroupFileValid, completeGroup, failWaitingScene, scanAllStates) + the PHASES phase-name constants the tiers compare `orchState.phase` against. Substantially narrower than the combined MediaFsmPort (27 members / 2 FSMs) and narrower than the O-6-measured "14 ops" video half (setMerging/allGroupsDone are host-only internals the tiers never touch; the O-6 count included them). **9 call sites in 3 files** (5 pre-counted lazy require sites + the derived PHASES reads).

**Error semantics (measured, preserved exactly):**
- `checkStalledVideoScenes` and `recoverVideoOrchStates` (reconciliation-engine) wrap the require in `try { … } catch (_) { return 0; }` — a missing module is NOT a stalled/recoverable state. This optional-load contract is preserved by resolving the ADAPTER through the port inside the same try/catch (`require('./video-fsm-port').videoFsm()` throws when the port is unwired; if the host service itself cannot be loaded, the adapter's lazy call-time resolver throws at the same point the pre-O-8 require did — the PHASES read sits inside the same try/catch). No mechanical fail-fast was introduced where it would change behavior.
- `checkVideoOrchInvariants` was a MANDATORY load (no try/catch) pre-O-8 and stays mandatory post-O-8.
- `scene-orchestrator`/`orchestrator` lazy requires were mandatory-at-call-time and stay mandatory-at-call-time through the fail-fast port.
- The recovery `switch` compares literal phase strings ('GENERATING'/'WAITING_CHUNKS'/'MERGING'/'DONE'/'FAILED'), unchanged — the port never rewrites string comparisons into constants.

**Seam (all criteria satisfied):** one independent FSM (single Redis state machine `animastor:video-orch:*`, single phase machine NEW→GENERATING→WAITING_CHUNKS→MERGING→DONE/FAILED) · contract ≪ MediaFsmPort · existing `services/video-orchestrator` stays the host-side implementation unchanged · pure 1:1 delegation (identical args/returns/errors, including the `{ success, state, reason }` transition results, the `{ completed, reason, missing }` completeGroup shape and the `{ failed, missing }` failWaitingScene shape) · no business-logic move · no audio FSM, no Redis key grammar, no transition-map internals, no group-file validation threshold, no merge/source-cap pipeline in the port (phase NAMES are the FSM's public vocabulary; their STORAGE + the group-file/merge knowledge stay adapter-side).

**Candidates evaluated (decision rule: existing host owner / multiple real consumers / small stable contract / no god-interface / no business-logic move / 1:1 delegation / error+optional-load semantics preserved / no new SCC / real coupling cut):**

| Candidate | Verdict | Why |
|---|---|---|
| **VideoFsmPort** (services/video-orchestrator) | **SELECTED** | Existing host owner; 3 real consumers across BOTH tiers (13 ops + PHASES ≪ 27-member MediaFsmPort rejection threshold); 1:1 delegation; optional-load semantics mirror-able exactly (the O-7 audio pattern); kills the last media-FSM host-service edge in both tiers — the §32.23-sanctioned unblock path (a) second half |
| O-P5 OrchestrationConfigPort (config/runtime-config) | REJECTED (again — §32.23 verdict stands) | ~15 heterogeneous members across 6 files (TIMEOUTS slices, OUTPUT_DIR ×6 sites, HUB_URL, API key, REDIS keys, WORKER_HEARTBEAT set, WINDOW_SIZE, shareFeaturesEnabled()) — a god-interface by construction; no small stable contract; would need a per-concern split (WorkerHeartbeatKeys/Timeouts slices) each still lacking multi-consumer narrowness |
| O-P6 MediaPlanningPort (audio/image/video media modules) | REJECTED (again) | 6 files of media-implementation knowledge (segments/chunks math, duration probing, canonical-PNG resolution) — not a stable contract; larger than one seam |
| fs/path lifecycle seams (scene-restoration fs, OUTPUT_DIR env sites) | REJECTED | Single-file or already-folded edges (§32.7); no multiple real consumers; OUTPUT_DIR folds into O-P5 which is itself rejected |
| gpu-dispatcher PG repos | REJECTED | Pinned host transport inside the transport itself (§32.19) — its own future step, not a tier seam |

**Implementation:**
- `backend/src/runtime/video-fsm-port.js` — zero-require tier-owned contract: 13-op OPS list, PHASE_CONSTANTS vocabulary (NEW, GENERATING, WAITING_CHUNKS, MERGING, DONE, FAILED — no audio PLACEHOLDER_READY), `setVideoFsmPort` wiring gate (fail-fast on missing ops/PHASES), `videoFsm()`/`videoFsmOp()`/`videoFsmPhases()` call-time resolvers, `_resetVideoFsmPort` test hygiene.
- `backend/src/storage/video-fsm-adapter.js` — the single bridge: lazy call-time `require('../services/video-orchestrator')` (require.cache stubbing parity for the orchestration-stabilization purge-list harness), 13 × 1:1 delegations, `PHASES` passthrough getter.
- Consumers migrated (no parallel refactors): scene-orchestrator (executeVideoDispatch whole-adapter handle), orchestrator (video leg only via `videoFsmOp('setFailed')`), reconciliation-engine ×3 sites (watchdog + recovery optional-load whole-adapter handles; invariant check mandatory handle). All 5 former `services/video-orchestrator` tier require edges deleted.
- Composition root `backend.cjs` wires `setVideoFsmPort(storage/video-fsm-adapter)` after O-7, before the first top-level tier require; `tests/generation-test-bindings.cjs` mirrors it (order O-2 → O-3 → O-4 → O-5 → O-7 → O-8).
- Guards: `backend/tests/architecture/o8-video-fsm-port.test.js` O8-G1..G10 — zero tier host-service requires (static), zero-require port, exact frozen 13-op + PHASES contract with live-consumer minimality (both sanctioned consumption styles: `videoFsmOp('…')` and the whole-adapter `videoFsm()` handle with a bare-PHASES-alias provenance check), no FSM-implementation leakage (frozen adapter export set, no audio FSM, no PREFIX/key/createState/transitionState/MIN_VIDEO_BYTES), composition-root-before-consumer wiring + O-2→O-5→O-7→O-8 order, no dynamic bypass (tier proximity scan + adapter stays the single lazy channel), O-1/O-2/O-3/O-4/O-5/O-7 non-regression, runtime→orchestration = 0, adapter as the single new bridge with the frozen pre-O-8 host consumer set (routes/generation-routes.cjs, services/task-handler.cjs).

**Graph after (re-measured):** nodes 249 (+port, +adapter), lazy edges 155 (−5 tier→service, +6 tier→port call sites incl. PHASES reads, +1 adapter→service at lazy depth — net 0), runtime→orchestration 0, orchestration→storage 0, runtime→storage 3 (pinned), `runtime/orchestration → video-orchestrator = 0` with the single host-side `storage/video-fsm-adapter → services/video-orchestrator` edge, SCCs >1 node: still exactly the 1 pre-existing host-side pair. VERDICT: PASS on all checks.

**Tests (this worktree vs identical-command baseline worktree at f059629f):** O-8 guards 10/10 NEW; test:arch 919 (= 909 baseline + 10, 0 failing); O-2 6/6, O-3 10/10, O-4 10/10, O-5 10/10, O-7 10/10 (46 combined), O-G 10/10, S-5 7/7, dependency-guardrails green; tier/host suites green: video-orchestrator + reconciliation-engine (76), fail-stage solo 6/6, orchestration-stabilization + scope-slide + video-action-polish/reconciliation + counter-reconciliation (44), happy-path + image-orphan-generating-repair + dispatch-meta-lease-lifecycle (116), execute-lifecycle + generation-progress + worklist-rebuild + stage-dispatch-lifecycle (63), video-tokens + video-timeline + video-workflows + gen-scope + runtime-timeouts (134), generation-routes + cancellation-recovery + audio-orchestrator (38), generation-provider-seam (15), phase7-extraction-readiness + phase5-runtime-result (43). The combined video-orchestrator+reconciliation+fail-stage file-order run has the same 5 fail-stage failures on BOTH trees (require.cache interference between those suites' orchestrator.js stubbing strategies — reproduced 1:1 at the pristine f059629f baseline worktree, not an O-8 regression; fail-stage is 6/6 green solo on both trees). No GitHub status available for this run.

**Not done (out of scope, per mandate):** no physical move; no audio-FSM merge (MediaFsmPort rejection stands); O-P5 OrchestrationConfigPort (god-interface, rejected §32.23 — per-concern splits remain the only path); O-P6 MediaPlanningPort; O-P10 HubCancelPort (slot O-7b); Generation API, VBook API, HTTP/Redis/FSM/SQL semantics untouched; `services/video-orchestrator` public API unchanged (routes/task-handler consumers keep working).

**Final verdict: O-8 DONE — READY FOR NEXT EXTRACTION STEP.** The per-media-FSM split of O-P3 is COMPLETE (audio at O-7, video at O-8): both tiers now hold ZERO media-FSM host-service edges. The next extraction step per §32.14 remains the O-7b HubCancelPort slice (narrowest remaining candidate) or the physical move gate review (§32.13 tree) — each its own single-seam step.

### 32.26 O-9 — HubCancelPort extraction seam (DONE)

**Date:** 2026-09-13 · **Baseline:** 71d85aba (branch c21.4-physically-extract-analysis-from-backend) · **Single-seam step, behavior-neutral.**

**Mandate:** the §32.14 O-7b slot (O-P10 HubCancelPort — "narrowest remaining candidate" per the §32.25 final verdict) — hub-queue cleanup out of domain code as ONE seam step. Reconnaissance first; the seam was selected only after re-measuring the full graph and re-verifying O-1..O-8.

**Reconnaissance (re-measured at O-9 on 71d85aba, not trusted from docs):** the full static+lazy graph via `scripts/o3-scc-audit.cjs` confirmed the §32.25 baseline (249 nodes, 155 lazy edges, runtime→orchestration 0, orchestration→storage 0, runtime→storage 3 pinned gpu-dispatcher host transports, 1 pre-existing host-side SCC `services/workspace-ai-provider ↔ services/system-ai`). O-1..O-8 boundaries re-verified green BEFORE the seam decision (73 guards: O-2 6/6, O-3 10/10, O-4 10/10, O-5 10/10, O-7 10/10, O-8 10/10, O-G 10/10, S-5 7/7). Remaining tier→host hub-HTTP edges measured call-site by call-site:

| Tier consumer | Call sites | Operations actually used |
|---|---|---|
| `orchestration/orchestrator.js` (resetScenes cancel leg) | 1 call site (line 520) — through a lazy `require('../runtime/dispatch-engine')` handle (line 505, kept for `clearLeasesForScenes` above) | `clearHubDispatches(dispatchIds, { context, warn })` |
| `runtime/reconciliation-engine.js` (RELEASE_STALE_LEASE recovery leg) | 1 call site (line 1246) — through the lazy `require('./dispatch-engine')` handle (line 1219, kept for `cancelActiveDispatch` above), wrapped in `try { … } catch (hubErr) { warn }` best-effort | `clearHubDispatches(clearedDispatchIds, { context })` |

**Union:** ONE op — `clearHubDispatches(dispatchIds, options)` → `{ requested, cleared, failed }` (per-id best-effort: an id failing HTTP/network counts into `failed` with a `warn` and never throws the loop). The host implementation (`runtime/dispatch-engine.js` clearHubDispatches — hub URL/API-key resolution from runtime-config, the `DELETE /queue/clear?dispatch_id=` endpoint shape, optional `hubUrl`/`apiKey`/`warn`/`fetchImpl` test overrides) stays in place unchanged. Out-of-tier callers (routes/generation-routes.cjs cancel-worker, services/entity-cleanup.cjs, backend tests gpu-hub-cleanup) keep calling the host export directly — host-side by design.

**Error semantics (measured, preserved exactly):**
- The reconciliation RELEASE_STALE_LEASE leg wraps the call in `try { … } catch (hubErr) { warn(…non-fatal…) }` — hub cleanup failure must not block state recovery. Preserved verbatim: the try/catch now wraps the PORT call (`hubCancelOp('clearHubDispatches')` throws when the port is unwired or the engine call rejects — the caller's catch handles both exactly as it handled a direct engine rejection before).
- The orchestrator resetScenes leg was a plain mandatory `await` pre-O-9 and stays a plain mandatory `await` through the fail-fast port (its `{ context: 'RESET-SCENES', warn }` options propagate verbatim).
- No mechanical fail-fast was introduced where the caller previously tolerated failure, and none removed where the caller previously crashed.

**Seam (all criteria satisfied):** one narrow host-owned operation · 2 real tier consumers across BOTH tiers (orchestration resetScenes, runtime reconciliation) · contract is ONE op (≪ every prior seam; the anti-god-interface criterion satisfied trivially) · existing dispatch-engine implementation stays the host-side owner unchanged · pure 1:1 delegation (identical args/returns/errors incl. the `{ requested, cleared, failed }` accounting and per-id warn behavior) · no business-logic move · no HUB_URL/API-key/fetch/endpoint knowledge crosses the port · no lease-cancellation ops ride the port (cancelActiveDispatch/clearLeases* stay direct dispatch-engine calls — Redis domain operations, not hub HTTP) · no Redis/PG/HTTP semantics change.

**Candidates evaluated (decision rule: existing host owner / multiple real tier consumers / small stable contract / no god-interface / no business-logic move / 1:1 delegation / error+optional-load semantics preserved / no new SCC / real coupling cut):**

| Candidate | Verdict | Why |
|---|---|---|
| **HubCancelPort** (O-P10, dispatch-engine.clearHubDispatches) | **SELECTED** | Existing host owner; 2 real tier consumers across BOTH tiers; ONE-op contract (the narrowest of the whole O-series); 1:1 delegation; best-effort semantics mirror-able exactly; closes the last hub-HTTP edge in domain code — the §32.14-planned O-7b slot, §32.25-named "narrowest remaining candidate" |
| O-P5 OrchestrationConfigPort (config/runtime-config, 6 tier files) | REJECTED (§32.23 verdict stands) | ~15 heterogeneous members (TIMEOUTS slices, OUTPUT_DIR, HUB_URL, API key, REDIS keys, WORKER_HEARTBEAT set, WINDOW_SIZE, shareFeaturesEnabled()) — god-interface by construction; per-concern splits (WorkerHeartbeatKeys/Timeouts) each lack multi-consumer narrowness today |
| O-P6 MediaPlanningPort (audio/image/media modules) | REJECTED (again) | 6 files of media-implementation knowledge (segments/chunks math, music-metadata probing, canonical-PNG resolution) — not a stable contract; porting it = moving business logic |
| Layer-config accessor port (services/layer-config, 3 tier requires) | REJECTED | `layerConfig.get` is a config read (folds into O-P5 per §32.7 — the rejected god-interface); consumers span 2 tiers but the op surface would have to grow to be a real port; not standalone per §32.7 |
| Gen-scope accessor port (services/gen-scope, 1 tier file) | REJECTED | Exactly ONE tier consumer (scene-window) — criterion B (multiple real consumers) fails; §32.7 folds it into O-P5/O-P2 |
| gpu-dispatcher PG routing repos (runtime→storage, 3 lazy edges) | REJECTED | Pinned host transport inside the transport itself (§32.19) — gpu-dispatcher STAYS host-side at the physical move (§32.13); its repos are host internals, not a tier seam |
| Lifecycle/worker-coordination port (runtime-loop, worker-health, startup-resume) | REJECTED | Already composition-root-only (§32.8: timers/process/Redis-client construction never enter the tiers — O-G2/O-G3 pin it); no tier→host edge exists to close |
| Lease-cancellation port (cancelActiveDispatch/clearLeases* family) | REJECTED | Pure Redis-key domain operations INSIDE runtime — they are the domain's own state machine (lease+quota+renewal), not a host service; a port would move the domain's core, not cut a host boundary |

**Implementation:**
- `backend/src/runtime/hub-cancel-port.js` — zero-require tier-owned contract: ONE-op OPS list (`clearHubDispatches`), `setHubCancelPort` wiring gate (fail-fast on missing op), `hubCancel()`/`hubCancelOp()` call-time resolvers, `isHubCancelPortWired`, `_resetHubCancelPort` test hygiene.
- `backend/src/storage/hub-cancel-adapter.js` — the single bridge: lazy call-time `() => require('../runtime/dispatch-engine')` (require.cache stubbing parity for the gpu-hub-cleanup/reconciliation harnesses), 1 × 1:1 delegation. Frozen one-op export set.
- Consumers migrated (no parallel refactors): orchestrator.js resetScenes cancel leg (`hubCancelOp('clearHubDispatches')`, `{ context: 'RESET-SCENES', warn }` verbatim; the leg's own lazy dispatch-engine handle stays for `clearLeasesForScenes` — lease domain), reconciliation-engine.js RELEASE_STALE_LEASE leg (port call inside the unchanged best-effort try/catch; the leg's dispatch-engine handle stays for `cancelActiveDispatch`). Both former tier `dispatchEngine.clearHubDispatches` call edges deleted.
- Composition root `backend.cjs` wires `setHubCancelPort(storage/hub-cancel-adapter)` after O-8, before the first top-level tier require; `tests/generation-test-bindings.cjs` mirrors it (order O-2 → O-3 → O-4 → O-5 → O-7 → O-8 → O-9).
- Guards: `backend/tests/architecture/o9-hub-cancel-port.test.js` O9-G1..G10 — zero tier `clearHubDispatches` call sites on dispatch-engine handles (static code scan), zero-require port with no hub-transport knowledge (no HUB_URL/API key/fetch/endpoint), frozen ONE-op contract with live-consumer minimality (both tier call sites resolve through `hubCancelOp`), no hub-transport leakage (frozen adapter export set; adapter reaches ONLY dispatch-engine; dispatch-engine keeps its full public API), composition-root-before-consumer wiring + O-2→…→O-8→O-9 order + bindings mirror, no dynamic hub-cancel bypass (tier proximity scan + adapter stays the single lazy channel), O-1/O-2/O-3/O-4/O-5/O-7/O-8 non-regression, runtime→orchestration = 0, adapter as the single new bridge with the frozen pre-O-9 dispatch-engine consumer set (12 files incl. the adapter) and no reverse edge.
- Targeted runtime tests: `backend/tests/hub-cancel-port.test.js` (7) — port fail-fast contract (unwired/wiring-gate/half-wired), real-engine empty-ids delegation (no network), require.cache stubbing parity through the adapter, consumer-leg error semantics (reconciliation best-effort throw-through preserved), dispatch-engine frozen host surface.

**Graph after (re-measured via `scripts/o3-scc-audit.cjs`):** nodes 251 (+port, +adapter), lazy edges 156 (+1 net: −2 tier→dispatch-engine hub-cancel call edges, +2 tier→port call-site edges, +1 adapter→dispatch-engine lazy edge), runtime→orchestration 0, orchestration→storage 0, runtime→storage 3 (pinned gpu-dispatcher host transports, unchanged), tier `clearHubDispatches` call sites on host handles = 0 with the single host-side `storage/hub-cancel-adapter → runtime/dispatch-engine` edge, SCCs >1 node: still exactly the 1 pre-existing host-side pair (`services/workspace-ai-provider ↔ services/system-ai`). VERDICT: PASS on all checks.

**Tests (this worktree):** O-9 guards 10/10 NEW; test:arch 929 (= 919 baseline + 10, 0 failing); O-2 6/6, O-3 10/10, O-4 10/10, O-5 10/10, O-7 10/10, O-8 10/10 (66 combined), O-G 10/10, S-5 7/7, dependency-guardrails green. Targeted consumer/host suites green: gpu-hub-cleanup + stale-lease-semantics + scope-slide + dispatch-meta-lease-lifecycle (68), reconciliation-engine (55), generation-routes (6), orchestration-stabilization (11), happy-path (75), image-ghost-no-jobs-sent + cancellation-recovery.integration (32), fail-stage + execute-lifecycle + stage-dispatch-lifecycle + worklist-rebuild (47), hub-cancel-port targeted suite (7). No GitHub status available for this run.

**Not done (out of scope, per mandate):** no physical move (the O-9 slot of §32.14 moves to the next task: `packages/animastor-orchestration` per the §32.13 tree, gated on the O-2..O-9 guard suite staying green); O-P5 OrchestrationConfigPort (god-interface rejection stands — only per-concern splits can unblock it); O-P6 MediaPlanningPort; layer-config/gen-scope accessors (fold into O-P5/O-P2 per §32.7); gpu-dispatcher PG routing repos (host transport, stays at the move); Generation API, VBook API, HTTP/Redis/PG/FSM/SQL semantics untouched; `dispatch-engine` public API unchanged (routes/entity-cleanup/test consumers keep working).

**Final verdict: O-9 DONE — READY FOR THE PHYSICAL MOVE GATE REVIEW.** HubCancelPort (the §32.14 O-7b slot) is extracted: both tiers now hold ZERO hub-HTTP edges — hub-queue cleanup happens ONLY through runtime/hub-cancel-port. All §32.14 pre-move seams are now closed (O-2 persistence, O-3 scene-data, O-4 placeholder-audio, O-5 progress-events, O-7 audio-FSM, O-8 video-FSM, O-9 hub-cancel; O-P8/O-P9 remain deliberately as the existing orchestration-seams registry). The next extraction step per §32.14 is the physical move itself (gate: full O-G suite + S5/S6/G7 regression + full-suite parity vs a pristine baseline worktree).

### 32.27 O-10 — LayerConfigPort extraction seam (DONE)

**Date:** 2026-09-13 · **Baseline:** 47f5b27f (branch c21.4-physically-extract-analysis-from-backend) · **Single-seam step, behavior-neutral.**

**Mandate:** the next dependency/seam audit for `backend/src/runtime/**` + `backend/src/orchestration/**` after O-1..O-9 — determine whether ONE more safe host-boundary seam exists (reconnaissance-first; do not create a seam for the sake of progress; config must be decomposed by concern, no automatic OrchestrationConfigPort).

**Reconnaissance (re-measured at O-10 on 47f5b27f, not trusted from docs):** the full static+lazy graph via `scripts/o3-scc-audit.cjs` confirmed the §32.26 baseline (251 nodes, 156 lazy edges, runtime→orchestration 0, orchestration→storage 0, runtime→storage 3 pinned gpu-dispatcher host transports, 1 pre-existing host-side SCC `services/workspace-ai-provider ↔ services/system-ai`). O-1..O-9 boundaries re-verified green BEFORE the seam decision (73 guards: O-2 6/6, O-3 10/10, O-4 10/10, O-5 10/10, O-7 10/10, O-8 10/10, O-9 10/10, O-G 10/10, S-5 7/7; test:arch 929 baseline, 0 failing). Remaining tier→host edges re-measured call-site by call-site:

| Contour | Tier edges | Members the tiers actually use |
|---|---|---|
| `services/layer-config` | 3 requires (scene-orchestrator top-level; reconciliation-engine ×2 lazy — PHASE C6 + rebuildWorkList) | `get(redis, bookId)`, `restoreFromBooks(redis)` — TWO ops, both real |
| `config/runtime-config` | 6 (dispatch-engine, gpu-dispatcher†, worker-health, scene-window, reconciliation-engine, orchestrator cancel leg) | OUTPUT_DIR ×8 sites, TIMEOUTS.{AUDIO_CHUNK_STALL_MS,VIDEO_CHUNK_STALL_MS,IU_IN_FLIGHT_TTL_S,FORCE_DISPATCH_TTL_S}, HUB_URL, GPU_HUB_API_KEY, GPU_TIMEOUT_MS, REDIS.CLEANUP_LOCK, WORKER_HEARTBEAT_{KEY,TYPE_PATTERN,TTL,TYPES}, WINDOW_SIZE, shareFeaturesEnabled() — ~15 heterogeneous members, 6 files (†pinned host transport) |
| `services/gen-scope` | 1 (scene-window) | getScope, scopeBounds |
| media modules (O-P6) | 6 files | segments/chunks math, music-metadata probing, canonical-PNG resolution — media implementation knowledge |
| gpu-dispatcher PG repos | 3 (inside gpu-dispatcher itself) | pinned host transport (§32.19) |
| orchestration `process.env.OUTPUT_DIR` | 3 sites (scene-restoration ×1, scene-callbacks ×2) | O-G9 pinned env leak |
| state/** | 5 files | S-5 seam boundary — owned, deliberately NOT a port |
| orchestration→runtime dispatch-engine/scheduler | ~15 edges | domain-internal (same-package at the move), not host boundaries |

**Config decomposition by concern (mandated check — no automatic OrchestrationConfigPort):** the 6-file runtime-config usage decomposes as: (a) fs/output roots — OUTPUT_DIR (runtime ×5 files/8 sites via `config.OUTPUT_DIR`, orchestration ×3 RAW env sites O-G9-pinned + orchestrator cancel leg ×1) — immutable process config; (b) hub credentials — HUB_URL/GPU_HUB_API_KEY/GPU_TIMEOUT_MS (dispatch-engine, gpu-dispatcher — both stay host-side at the move per §32.13); (c) Redis key grammar — REDIS.CLEANUP_LOCK (reconciliation ×1) + WORKER_HEARTBEAT_* family (worker-health ×6 reads; routes read the same family ×8 — HOST-majority); (d) timeout scalars — TIMEOUTS slices (dispatch-engine ×1, reconciliation ×2, orchestrator ×1 — 1:1 with `gpu-dispatcher`'s jobMs routing); (e) behavior toggles — shareFeaturesEnabled() lazy env kill-switch (worker-health, gpu-dispatcher — env-dependent call-time semantics a snapshot port would freeze, §32.23); (f) WINDOW_SIZE (scene-window ×1 file). NO single concern slice has BOTH multi-tier consumers AND a narrow stable contract AND real boundary value: (a) is the biggest but its orchestration half is the O-G9-pinned env leak whose closure changes bookkeeping, its runtime half is served by the single `config.OUTPUT_DIR` destructure, and both would still need the same value at the move (host-supplied composition-root injection is the §32.13 plan, not a port); (b) never enters the package (dispatch-engine/gpu-dispatcher host-side); (c) WORKER_HEARTBEAT is HOST-majority (routes ×8 vs tier ×1 file) — an accessor with one real tier consumer; (d) TIMEOUTS slices each have 1 tier consumer per slice; (f) one consumer. **Config verdict: no safe split today — the §32.23 O-P5 god-interface rejection STANDS; per-concern splits each fail the multi-consumer/boundary-value test.**

**Candidates evaluated (decision rule: existing host owner / multiple real tier consumers / small stable contract / no god-interface / no business-logic move / 1:1 delegation / error+optional-load semantics preserved / no new SCC / real coupling cut / not a rename of an existing call / not already closed by O-1..O-9):**

| Candidate | Verdict | Why |
|---|---|---|
| **LayerConfigPort** (services/layer-config) | **SELECTED** | Existing host owner; TWO real tier consumers across BOTH tiers (scene-orchestrator executeVideoDispatch timeout leg, reconciliation-engine rebuildWorkList + PHASE C6) — the last remaining multi-consumer services/** host boundary; TWO-op contract (narrowest measured multi-consumer surface); pure 1:1 delegation; optional-load/try-catch semantics mirror-able exactly (the O-7/O-9 caller-owned pattern); kills the last 3 tier→services/layer-config require edges |
| O-P5 OrchestrationConfigPort (config/runtime-config, 6 tier files) | REJECTED (§32.23 verdict stands — confirmed by the per-concern decomposition above) | ~15 heterogeneous members (fs roots + hub credentials + Redis key grammar + timeout scalars + lazy kill-switch + window size) — god-interface by construction; NO concern slice passes the multi-consumer/boundary-value test; gpu-dispatcher is a pinned host transport |
| OrchestrationConfigPort per-concern splits (OUTPUT_DIR root port / WorkerHeartbeatKeys / TimeoutsPort) | REJECTED | OUTPUT_DIR: orchestration half is the O-G9-pinned env leak (closure = separate bookkeeping step, not a seam); runtime half = single `config.OUTPUT_DIR` destructure in 1 file each. WorkerHeartbeat: HOST-majority consumer set (routes ×8 vs tier ×1). Timeouts: 1 tier consumer per slice. Each = an accessor with one real tier consumer |
| gen-scope accessor port (services/gen-scope, scene-window ×1) | REJECTED | Exactly ONE tier consumer — criterion B fails (unchanged since §32.23/§32.26) |
| O-P6 MediaPlanningPort (audio/image/video modules) | REJECTED (again) | 6 files of media-implementation knowledge — not a stable contract; porting = moving business logic |
| gpu-dispatcher PG routing repos (runtime→storage, 3 lazy edges) | REJECTED | Pinned host transport inside the transport itself (§32.19) — stays host-side at the move |
| Lifecycle/worker-coordination port (runtime-loop, worker-health coordination, startup-resume) | REJECTED | Composition-root-only already (§32.8, O-G2/O-G3 pin it) — no tier→host edge to close |
| Lease-cancellation family (cancelActiveDispatch/clearLeases*) | REJECTED | Pure Redis-key domain operations INSIDE runtime (§32.26 verdict stands) |
| LayerConfigPort "raw Redis reads rider" (the 5 tier-local `animastor:layer-config:*` raw reads) | REJECTED (NOT in this seam) | They are bespoke partial-parse reads of an INJECTED redis parameter (per-field defaults, `JSON.parse`-tolerant) — moving them through normalize() would CHANGE semantics (the §32.26 lease-family logic: redis is the domain's own state, not a host service); pinned at their measured count by O10-G6 instead |
| SceneStateSink / DispatchExecutorPort (O-P8/O-P9) | REJECTED | Already exist as the orchestration-seams registry — a port would re-name the existing seam, not cut a new edge |

**Selected seam (all criteria satisfied):** one narrow host-owned service · 2 real ops consumed across BOTH tiers (scene-orchestrator, reconciliation-engine ×2 legs) · contract is TWO ops ≪ every prior multi-consumer seam · existing services/layer-config stays the host-side implementation unchanged · pure 1:1 delegation (identical args/returns/errors incl. the normalized config shape and the restore count) · no business-logic move · no Redis key grammar, no fs/BOOKS_DIR, no clamp pipeline, no durable book.json scan knowledge crosses the port · no Redis/PG/HTTP semantics change · no new SCC.

**Error semantics (measured, preserved exactly):**
- The reconciliation PHASE C6 leg previously wrapped the service require in an inner `try { … } catch (_) { layerConfig = null }` (missing module → phase silently skipped) and the CALL in an outer `try { … } catch (err) { warn; summary.errors }` (failing restore → non-fatal phase error). Post-O-10: the port call sits inside the SAME outer try/catch; the adapter's lazy call-time resolver throws at exactly the point the pre-O-10 inner require did, and a FAILING call still reaches the outer warn path. A `layer_config_restore:N` phase is still reported only on a successful non-zero restore. Verified by the layer-config-reconcile integration suite (4/4 green solo).
- The scene-orchestrator timeout leg keeps its `try { … } catch (_) {}` tolerance verbatim around the port call — a throwing read leaves `videoTimeoutMs` undefined and the dispatch proceeds (targeted test).
- The rebuildWorkList read stays a plain mandatory `await` through the fail-fast port.

**Implementation:**
- `backend/src/runtime/layer-config-port.js` — zero-require tier-owned contract: TWO-op OPS list (`get`, `restoreFromBooks`), `setLayerConfigPort` wiring gate (fail-fast on missing op), `layerConfig()`/`layerConfigOp()` call-time resolvers, `isLayerConfigPortWired`, `_resetLayerConfigPort` test hygiene.
- `backend/src/storage/layer-config-adapter.js` — the single bridge: lazy call-time `() => require('../services/layer-config')` (require.cache stubbing parity — the worklist-rebuild purge-list convention), 2 × 1:1 delegations. Frozen two-op export set.
- Consumers migrated (no parallel refactors): scene-orchestrator executeVideoDispatch timeout leg (`layerConfigOp('get')` inside the unchanged `try { … } catch (_) {}`), reconciliation-engine PHASE C6 (`layerConfigOp('restoreFromBooks')` inside the unchanged outer best-effort try/catch; the inner require-degrade try collapses into the same caller-owned catch) and rebuildWorkList (`layerConfigOp('get')`, plain mandatory await). All 3 former tier `services/layer-config` require edges deleted.
- Tier-local RAW Redis reads of `animastor:layer-config:*` (reconciliation video-stall threshold, scene-window ×3, runtime-scheduler getLayerConfig) deliberately NOT migrated: bespoke partial-parse semantics, not host-service edges; pinned by O10-G6.
- Composition root `backend.cjs` wires `setLayerConfigPort(storage/layer-config-adapter)` after O-9, before the first top-level tier require; `tests/generation-test-bindings.cjs` mirrors it (order O-2 → O-3 → O-4 → O-5 → O-7 → O-8 → O-9 → O-10).
- Guards: `backend/tests/architecture/o10-layer-config-port.test.js` O10-G1..G10 — zero tier `services/layer-config` requires (static), zero-require port with no key grammar/BOOKS_DIR/normalize knowledge, frozen TWO-op contract with live-consumer minimality (both ops consumed through `layerConfigOp`), no service-implementation leakage (frozen adapter export set; adapter reaches ONLY services/layer-config; service keeps its full public API), composition-root-before-consumer wiring + O-2→…→O-9→O-10 order + bindings mirror, no dynamic bypass (tier proximity scan + raw-Redis read count pinned at 5) + adapter stays the single lazy channel, O-1..O-9 non-regression, runtime→orchestration = 0, adapter as the single new bridge with the frozen pre-O-10 host consumer set (services/agent/bootstrap, services/agent/pipeline-runner, storage/index barrel; routes consume via deps injection, not require) and no reverse edge.
- Targeted runtime tests: `backend/tests/layer-config-port.test.js` (10) — port fail-fast contract (unwired/wiring-gate/half-wired), real-service delegation with frozen normalize/clamp semantics, require.cache stubbing parity through the adapter, consumer-leg error semantics (scene-orchestrator throw tolerance, C6 non-fatal phase error, missing-service degrade parity), dispatch-engine-frozen host surface analogue (service public API unchanged).

**Graph after (re-measured via `scripts/o3-scc-audit.cjs`):** nodes 253 (+port, +adapter), lazy edges 156 (−3 tier→service, +3 tier→port requires incl. the C6 inline require, +1 adapter→service at lazy depth — net +1 node-level, 0 net lazy delta vs the §32.26 count is +0: the baseline was 156 and stays 156), runtime→orchestration 0, orchestration→storage 0, runtime→storage 3 (pinned gpu-dispatcher host transports, unchanged), tier `services/layer-config` requires = 0 with the single host-side `storage/layer-config-adapter → services/layer-config` edge, SCCs >1 node: still exactly the 1 pre-existing host-side pair (`services/workspace-ai-provider ↔ services/system-ai`). VERDICT: PASS on all checks.

**Tests (this worktree):** O-10 guards 10/10 NEW; test:arch 939 (= 929 baseline + 10, 0 failing); O-2 6/6, O-3 10/10, O-4 10/10, O-5 10/10, O-7 10/10, O-8 10/10, O-9 10/10 (76 combined), O-G 10/10, S-5 7/7, dependency-guardrails green. Targeted consumer/host suites green: reconciliation-engine (55), worklist-rebuild (15), layer-config-reconcile C6 (4), layer-config + layer-config-persistence (34), scope-slide (20), scope-filter + gen-scope (40), happy-path (75), fail-stage + stage-dispatch-lifecycle + execute-lifecycle (47), layer-config-port targeted suite (10). The combined reconciliation+worklist+layer-config-reconcile file-order run keeps the SAME 3 layer-config-reconcile C6 failures as the PRISTINE 47f5b27f baseline worktree (require.cache interference between the worklist purge-list stubbing and the C6 real-service harness — reproduced 1:1 at baseline via git stash, each suite green solo; not an O-10 regression). No GitHub status available for this run.

**Not done (out of scope, per mandate):** no physical move (the §32.14 O-9/O-10 physical-move slot remains the next task: `packages/animastor-orchestration` per the §32.13 tree, gated on the O-2..O-10 guard suite staying green); O-P5 OrchestrationConfigPort (god-interface rejection STANDS — the per-concern decomposition above shows every slice fails the multi-consumer/boundary-value test); O-P6 MediaPlanningPort; gen-scope accessor (single consumer); the 5 tier-local raw Redis layer-config reads (pinned, bespoke semantics — a future seam only if their fallback semantics are first unified); gpu-dispatcher PG routing repos (host transport); Generation API, VBook API, HTTP/Redis/PG/SQL semantics untouched; `services/layer-config` public API unchanged (routes/agent/test consumers keep working).

**Final verdict: O-10 DONE — READY FOR THE PHYSICAL MOVE GATE REVIEW.** LayerConfigPort is extracted: both tiers consume per-book layer config ONLY through runtime/layer-config-port — the LAST remaining multi-tier-consumer `services/**` host boundary is closed. Remaining measured tier→host edges after O-10: config/runtime-config (6 files — O-P5 god-interface, per-concern splits all fail the multi-consumer test), services/gen-scope (1 file — single consumer), the O-G9-pinned orchestration env leak (3 sites), pinned gpu-dispatcher host transport (3 storage edges), media modules (O-P6 — forbidden business logic) and the S-5-owned state seam. NO further single seam satisfies the criteria — the next extraction step per §32.14 is the physical move itself (gate: full O-G suite + S5/S6/G7 regression + full-suite parity vs a pristine baseline worktree).

### 32.28 O-11 — next host-boundary seam reconnaissance audit (BLOCKED — no seam extracted)

**Date:** 2026-09-13 · **Baseline:** c44c136e (branch c21.4-physically-extract-analysis-from-backend) · **Audit-only step; production code untouched.**

**Mandate:** the next dependency/seam audit for `backend/src/runtime/**` + `backend/src/orchestration/**` after O-1..O-10 — determine whether ONE more safe host-boundary seam exists (reconnaissance-first; do not create a seam for the sake of progress; lifecycle/worker-coordination and dispatch-engine received the mandated special attention).

#### Re-measured graph (via `scripts/o3-scc-audit.cjs`, NOT trusted from docs)

Identical to the §32.27 post-O-10 baseline on every axis: **nodes 253, lazy edges 156, runtime→orchestration 0, services→orchestration 0, orchestration→book 0, runtime→book 0, orchestration→storage 0, runtime→storage 3 (all three inside `runtime/gpu-dispatcher.js` — the pinned host transport: book-repo/worker-repo/workspace-repo), SCCs >1 node = exactly the 1 pre-existing host-side pair (`services/workspace-ai-provider ↔ services/system-ai`).** No new nodes, no new edges, no new SCC, no bypass around O-2..O-10 ports (guarded by O10-G6-style proximity scans; the lazy-edge delta vs §32.27 is 0).

**O-1..O-10 regression check (run BEFORE the candidate decision):** 96 combined port-guard tests green (O-2 6/6, O-3 10/10, O-4 10/10, O-5 10/10, O-7 10/10, O-8 10/10, O-9 10/10, O-10 10/10, O-G 10/10 via runtime-orchestration-recon, S-5 7/7); full `test:arch` = **939 passing, 0 failing** (= the §32.27 baseline count). Closed-port zero-bypass re-verified by grep + guards: tier requires of `services/layer-config`, `services/audio-orchestrator`, `services/video-orchestrator`, `services/placeholder-audio`, `services/progress-pubsub|generation-progress`, `book/**`, `storage/**` (outside pinned gpu-dispatcher) = 0 in both tiers; no new dynamic/lazy edges around closed seams (computed-require scan finds only `runtime/index.js` lazyRequire — unchanged since §32.20).

#### Remaining tier→host inventory (every edge, measured call-site by call-site)

| # | Host dependency | Tier consumers (N files) | Ops actually used | Direct edges | Optional-load/error semantics | Host-boundary seam? |
|---|---|---|---|---|---|---|
| 1 | `config/runtime-config` | 6 (orchestrator, dispatch-engine, gpu-dispatcher†, worker-health, scene-window, reconciliation-engine) | ~15 heterogeneous members (see §32.27 per-concern decomposition — unchanged) | 7 requires (orchestrator ×1 lazy) | plain load-time reads + `shareFeaturesEnabled()` call-time env kill-switch | NO — §32.23/§32.27 god-interface rejection STANDS; every per-concern slice has ≤1 real tier consumer per concern |
| 2 | `services/gen-scope` | 1 (scene-window) | `getScope`, `scopeBounds` (2 ops; setScope/clearScope/migrateLegacyScopes/inScope consumed only by host routes/services via `storage/index` barrel) | 1 | plain mandatory `await` | NO — single tier consumer, criterion B fails (3rd consecutive audit) |
| 3 | Redis (as injected parameter) | all tiers | domain-state key grammar — runtime's OWN state per §32.26 lease-family verdict | 0 direct client requires (O-G2 green) | n/a | NO — redis is the domain's own state, not a host service |
| 4 | PostgreSQL | 0 outside pinned transport | — | 0 (all PG inside `gpu-dispatcher` + PersistencePort adapter) | — | NO — O-2 closed; the 3 remaining runtime→storage edges are inside the §32.19-pinned host transport file |
| 5 | `process.env` / config | orchestration OUTPUT_DIR ×3 sites (O-G9-pinned); runtime env = 0 | `process.env.OUTPUT_DIR \|\| '/data/output'` | 3 raw env reads | fallback-default | NO (as a seam) — O-G9 pins the leak at 3; closure is a bookkeeping/env-injection change (§32.13 plan: host-supplied composition-root injection), NOT a port — moving it would rename a constant, not cut a boundary |
| 6 | dispatch-engine / GPU Hub | tier-internal: orchestration→`runtime/dispatch-engine` (~6 ops: dispatchStage via seams, finalizeDispatch ×2, verifyDispatchIdentity ×2, clearLeasesForScenes) + `runtime/gpu-dispatcher` (transport, stays host-side §32.13); hub-HTTP in domain code = 0 (O-9) | all within the future package — same-package edges at the move | ~15 intra-tier edges | fail-fast ports (O-9 hub-cancel wired) | NO — orchestration→runtime is domain-internal (both move into the package together per §32.13); gpu-dispatcher is the pinned host transport; no remaining hub-HTTP/fetch edge outside it |
| 7 | lifecycle / worker coordination | composition-root only: `backend.cjs` (loop.start/stop/setReconcileDeps/isRunning ×5, startup `reconcileCycle({startup:true})`, `startup-resume.resumeIncompleteSessions` injected into reconcileDeps); tiers expose functions, host CALLS them | startup, worker registration (routes/worker-routes + worker-auth mirror — host), worker availability (worker-health — runtime-OWNED, intra-tier), reconciliation/scheduler startup, shutdown, background loops (runtime-loop timer shell — stays host §32.13) | 0 tier→host requires | host owns all ordering | NO — mandated special attention paid: every lifecycle edge is host→tier invocation (composition-root orchestration); there is NO tier→host lifecycle dependency to close. Runtime-owned lifecycle (worker-health heartbeat scan) is intra-tier; a "LifecyclePort" would port the composition root itself — explicitly forbidden ("Не выносить composition-root orchestration просто ради port") |
| 8 | cancellation | HubCancelPort (O-9 closed, hub-queue leg); `scene-window` cancel flags (Redis domain keys, intra-tier); generation-cancel-repo (host, PersistencePort channel); orchestrator resetScenes clearLeases+hub-cancel via O-9 port | all closed or domain-internal | 0 open | fail-fast port wired | NO — nothing new since O-9; clearLeasesForScenes/cancelActiveDispatch are runtime-OWNED Redis domain ops (§32.26 verdict: pure Redis-key domain operations INSIDE runtime) |
| 9 | scheduler | `runtime/runtime-scheduler` — runtime-OWNED, consumed by orchestration (intra-tier at the move: 6 require sites) + backend.cjs timer shell | tick/shouldSchedule/detectVersionStale/removeScenesFromActiveIndex | intra-tier only | — | NO — domain-internal (same-package at the move) |
| 10 | reconciliation | `runtime/reconciliation-engine` — runtime-OWNED; host calls it (backend.cjs startup + runtime-loop setReconcileDeps); its host deps flow through O-2/O-10 ports | — | intra-tier + host-injected deps | — | NO — same as #7 (composition-root orchestration) |
| 11 | other host-owned facades | `audio/**` (5 files: scene-orchestrator, scene-callbacks, reconciliation ×2+, scene-window), `image/**` (3 files), `video/**` (2 files), `music-metadata` (scene-callbacks ×2), `state/**` (12 files — S-5-owned seam, deliberately not a port), `metrics/prometheus` (runtime-loop), `contracts/runtime-result` (runtime-result-emitter — leaf contract below both tiers) | media planning/validation knowledge (segments/chunks math, music-metadata probing, canonical-PNG resolution) | ~14 media edges | several lazy with caller-owned catch | NO — O-P6 MediaPlanningPort = forbidden business-logic move (§32.23/§32.26/§32.27 rejections STAND); state/** is the S-5 seam boundary by design; contracts leaf cannot cycle; prometheus is a host observability leaf inside runtime-loop (stays host §32.13) |

† gpu-dispatcher is the pinned host transport (stays host-side at the move per §32.13).

#### Special attention #1 — lifecycle / worker coordination (post-O-10 check for a NEW narrow seam)

Mandated split performed. **Host lifecycle that must stay in composition root:** server startup ordering (backend.cjs `startServer`), workflow loading, PG init, session/guest purge timers, worker-auth mirror sync, post-listen `runtime.loop.start` + `setImmediate` startup `reconcileCycle`, graceful shutdown (`loop.stop`). All of these are host→tier INVOCATIONS with zero tier→host requires — a port would invert nothing. **Runtime-owned lifecycle:** runtime-loop tick body, scheduler, worker-health heartbeat availability scan, reconciliation engine — all intra-tier. **Genuinely reusable host service with a port:** none found — worker registration (routes/worker-routes + worker-setup-routes + worker-auth) is host-only; worker AVAILABILITY (`worker-health.isAvailable/getStatus/getAvailability`) is already runtime-OWNED (it lives inside `runtime/worker-health.js` — the tiers' and host's `helpers/redis-helpers.cjs`/routes consume IT, not vice versa); heartbeat key grammar (WORKER_HEARTBEAT_* family) is host-majority (routes ×5 sites vs tier ×1 file worker-health, re-measured: routes worker-setup-routes/admin-routes/generation-routes/worker-routes ×2 = 5 host sites) — the §32.27 accessor-with-one-real-tier-consumer rejection STANDS. **Rejection: no narrow stable lifecycle contract exists — the family is composition-root orchestration by construction.**

#### Special attention #2 — dispatch-engine after O-9

Remaining direct `runtime/orchestration → dispatch-engine` call sites split per the mandate: **host transport/infrastructure** — none outside `gpu-dispatcher` (pinned, host-side); **domain orchestration** — dispatchStage (via S-5 seams), finalizeDispatch, verifyDispatchIdentity, clearLeasesForScenes, cancelActiveDispatch — all domain-internal same-package edges at the move; **cancellation** — hub leg closed by O-9 (`hub-cancel-port` wired in orchestrator.js:523 + reconciliation STALE_LEASE), Redis lease legs are runtime-owned domain ops; **lease management** — lease-manager + dispatch-engine key grammar = runtime-owned pure functions (§32.26 verdict); **quota/circuit-breaker** — intra-tier (circuit-breaker.js, retry-budget-manager.js inside runtime); **worker coordination** — availability scans via worker-health (intra-tier) + gpu-dispatcher routing caches (pinned host transport). **No narrow host concern with multiple tier consumers remains → a "DispatchPort" stays forbidden and nothing narrower emerged.** No large DispatchPort created.

#### Candidate decision table (measured at O-11 on c44c136e)

| Candidate | Real consumers | Ops | Direct edges | Verdict |
|---|---|---|---|---|
| OrchestrationConfigPort (broad) | 6 tier files | ~15 members | 7 | REJECTED — god-interface (§32.23 STANDS, unchanged since §32.27) |
| Per-concern config splits (OUTPUT_DIR / WorkerHeartbeatKeys / Timeouts / WindowSize) | 1 per slice | 1–4 | 1–2 | REJECTED — each slice has exactly ONE real tier consumer (re-measured: OUTPUT_DIR runtime=scene-window+reconciliation+dispatch-engine via one destructure each but orchestration half is the O-G9 env leak; WORKER_HEARTBEAT host-majority ×5 routes sites vs 1 tier file; TIMEOUTS 1 consumer per slice) |
| GenScopePort (services/gen-scope) | 1 (scene-window) | 2 | 1 | REJECTED — single tier consumer, 3rd consecutive audit; the service's real consumer majority is host (routes setScope, storage barrel, backend.cjs migrateLegacyScopes) |
| LifecyclePort / WorkerCoordinationPort | 0 tier→host edges | — | 0 | REJECTED — composition-root orchestration; porting it is explicitly forbidden (special attention #1 above) |
| DispatchPort (broad) / dispatch sub-slices (transport / lease / quota / cancellation / worker-coord) | 0 open host concerns | — | 0 outside pinned transport | REJECTED — hub-HTTP closed at O-9; remaining dispatch-engine consumers are intra-package-at-the-move domain edges; special attention #2 above |
| MediaPlanningPort (O-P6: audio/image/video/music-metadata edges) | ~10 tier files | unbounded media knowledge | ~14 | REJECTED — forbidden business-logic move (§32.23 STANDS) |
| Raw-Redis layer-config reads (5 sites, O10-G6-pinned) | 3 tier files | partial-parse reads | 0 (injected redis) | REJECTED — §32.27 verdict STANDS: bespoke fallback semantics; a port would CHANGE error semantics, not cut a host boundary |
| gpu-dispatcher PG routing repos → port | 0 (inside pinned transport) | 3 repo calls | 3 | REJECTED — pinned host transport per §32.13/§32.19; moving = PG semantics change risk inside the transport itself |
| O-G9 env leak closure (OUTPUT_DIR injection) | 3 orchestration sites | 1 constant | 3 | REJECTED as a SEAM — it is the §32.13 composition-root injection plan (host passes OUTPUT_DIR at wiring), a rename/injection bookkeeping step, not a host-boundary port; no consumers/ops contract |
| state/** StatePort | 12 tier files | large domain surface | ~40 | REJECTED — the S-5 seam boundary is OWNED by design (orchestration-seams registry + state layer); a port would duplicate the existing seam |
| Audio/Video/Placeholder/Layer/Progress/SceneData/Persistence/HubCancel port widening | — | — | — | REJECTED — O-2..O-10 closed and frozen; widening = god-interface growth |

**Decision: BLOCKED.** No candidate simultaneously satisfies host-owned implementation + multiple real tier consumers (or a single-consumer edge with exceptional boundary value) + narrow stable contract + minimal ops + 1:1 delegation + zero-require port + no business-logic move + no Redis/PG/HTTP semantics change + no new SCC + real coupling reduction + no duplication of an existing port. After O-10 the multi-consumer services class is EXHAUSTED (measured: tier→`services/**` = 1 edge total, gen-scope, single consumer); every remaining tier→host edge is either a pinned transport, a forbidden business-logic surface, a composition-root concern, or a single-consumer accessor.

#### What must change for the next seam to become safe (recorded, not acted on)

1. **Physical move gate** (the §32.14 next slot): the O-2..O-10 guard suite is green — the prerequisites for `packages/animastor-orchestration` per §32.13 are met; the move itself will reclassify the intra-tier edges (dispatch-engine, scheduler, state) as package-internal and may surface NEW genuinely host-owned narrow concerns (e.g. worker-availability host queries from outside the package) worth a port then.
2. **O-G9 env leak**: if OUTPUT_DIR becomes composition-root-injected (§32.13 plan), the 3 orchestration env reads disappear WITHOUT a port — bookkeeping prerequisite for the move.
3. **Raw layer-config reads**: only if the 5 bespoke fallback parsers are first unified into ONE semantic shape would a read-port become a defensible seam (currently semantics-changing, forbidden).
4. **WORKER_HEARTBEAT grammar**: if host routes sites are consolidated behind a host facade, a narrow WorkerHeartbeatKeysPort could pass the multi-consumer test — currently host-majority.

#### Re-verified at close

All 96 combined port guards green, `test:arch` 939/0 (identical to §32.27 baseline), graph/SCC identical to the post-O-10 measurement (nodes 253 / lazy 156 / runtime→orchestration 0 / the single pre-existing host SCC), no production files changed in this step. Targeted confirmation suites green solo: scope-filter+gen-scope+scope-slide (60), reconciliation-engine (55), dispatch-meta-lease-lifecycle (17), worker-health+gpu-hub-cleanup+happy-path (77), layer-config family (10+15+4 solo), generation-cancel-repo+cancellation-recovery (10), happy-path+fail-stage+stage-dispatch-lifecycle+execute-lifecycle (122). The combined scope/reconciliation/dispatch file-order run's single RELEASE_STALE_LEASE failure was reproduced 1:1 in a PRISTINE c44c136e baseline worktree (require.cache interference between the scope suites and the reconciliation harness — each suite green solo; pre-existing at the O-10 commit, not an O-11 finding). **No GitHub status available for this run.**

**Final verdict: O-11 BLOCKED — audit-only.** The next extraction step per §32.14 is the **physical move gate review** (`packages/animastor-orchestration`, §32.13 tree; gate: full O-G suite + S5/S6/G7 regression + full-suite parity vs a pristine baseline worktree). No seam was created for the sake of progress.

### 32.29 Physical Move Gate — final readiness audit for `packages/animastor-orchestration` (READY)

**Date:** 2026-09-13 · **Baseline:** branch c21.4-physically-extract-analysis-from-backend · **Audit-only step; production code untouched** (new: one guard suite `backend/tests/architecture/physical-move-gate.test.js`).

**Mandate:** the FINAL PHYSICAL MOVE GATE per §32.14 — re-measure everything, audit hidden dependencies, freeze the public API and host adapters, prove dependency direction and move-set closure, and produce the exact step plan. The move itself stays FORBIDDEN in this step.

#### Re-measured graph (via `scripts/o3-scc-audit.cjs`, NOT trusted from docs)

Identical to the §32.28 baseline: **nodes 253, lazy edges 156, runtime→orchestration 0, orchestration→book 0, orchestration→storage 0, orchestration→services 0, SCCs >1 node = exactly the 1 pre-existing host-side pair (`services/workspace-ai-provider ↔ services/system-ai`)**. No new nodes, no new edges, no new SCC. Move-set closure was proven independently (below): **every static+lazy require of every moved file resolves inside the move set, to a builtin/external, or to the frozen host-stays allowlist — zero edges outside it.**

#### Move-set inventory (measured closure, `node` walk of require graph — 28 files)

**Move (21 runtime + 7 orchestration):** runtime: `active-scenes-index, audio-fsm-port, circuit-breaker, counter-reconciliation, dispatch-engine, failure-taxonomy, hub-cancel-port, layer-config-port, lease-manager, persistence-port, placeholder-audio-port, progress-events-port, reconciliation-engine, retry-budget-manager, runtime-metrics, runtime-result-emitter, runtime-scheduler, scene-data-port, scene-window, video-fsm-port, worker-health`; orchestration: all 7 (`index, orchestrator, runtime-result-consumer, scene-callbacks, scene-orchestrator, scene-restoration, scene-utils`).

**Stays host-side (classified, 7 runtime files):** `gpu-dispatcher.js` (§32.19 pinned transport: Protocol v2 send + PW-2 routing caches + 3 PG routing repos), `runtime-loop.js` (§32.13 pinned timer shell), `index.js` (host facade barrel, single consumer `backend.cjs:154` — re-points to the package root), `orchestration-seams.js` (S-5 inbound seam registry — stays and binds the package API), `job-schema.js` (Phase 9C contracts facade — moved consumers switch to `@animastor/contracts` directly), `runtime-persistence.js` + `retention-manager.js` (dead — deleted in the post-move cleanup commit, NOT moved).

**Host-stays allowlist the move set may reach (exhaustive, 31 targets):** 8 port adapters + `storage/index` (`runtime-persistence-, scene-data-, placeholder-audio-, progress-events-, audio-fsm-, video-fsm-, hub-cancel-, layer-config-adapter`), `config/runtime-config` + `config/generation-config-adapter` (§32.23 no-port rejection STANDS), state/** ×5 (`index, scene-state, asset-state-store, scene-state-ops, event-journal` — S-5 seam boundary by design), `runtime/orchestration-seams` (inbound channel), `contracts/runtime-result` (leaf), media facades ×9 (`audio/index, audio-service, segments, chunks; image/index, image-service, iu-processor; video/index, video-service` — O-P6 rejection STANDS), `services/gen-scope` (single consumer scene-window), `runtime/job-schema` + the two DECLARED PRE-MOVE EXCEPTIONS: `runtime/gpu-dispatcher` (scene-window's single lazy PW-2 workspace-routing call site — MG-F-frozen; becomes a composition-root-injected resolver at the move, step 4b) and `runtime/runtime-persistence` (scheduler's dead `initializeRuntime` body only — MG-G-frozen at exactly one require site, zero callers; the export is dropped at the move).

**Hidden-dependency audit (measured):** `process.env` = the 3 O-G9-pinned OUTPUT_DIR reads in orchestration ONLY (runtime tier = 0); `__dirname`/`require.cache` in the move set = 0 code sites (the one grep hit, reconciliation-engine:2172, is a comment — MG-E strips comments and pins 0); timers = lease-manager's own (package-internal, move with it) vs runtime-loop's tick (host shell, stays); dynamic/computed requires = none reaching outside the package (MG-B proximity scan); fs usage = artifact-IO + scene-window disk-status only (O-G9 scope, unchanged); no module-level init ordering, no global state, no test-only bindings inside the move set (17 test files bind via `helpers/wire-seams.js` + the seams registry, which stay host-side and re-point once).

#### Public API freeze — `@animastor/orchestration`

**Public (host-facing, exactly today's consumption surface — nothing added "just in case"):** root facade = current `orchestration/index.js` exports verbatim: `ensureStageDispatchable, dispatchStage, restoreSceneChunkStatus, handleAudioCompleted, handleImageCompleted, handleVideoCompleted, completeStage, failStage, markDirty, markDirtyScene, planScene, beginStage, completeStageWithoutVideo, completeStageWithoutImage, setScenePending, setSceneGenerating, setSceneAllReady, setScenePlaceholder, rollbackStageToPending, reconcile, resetScenes` (+ the existing `orchestrator` namespace re-export kept during the shim phase) — plus `createRuntimeResultConsumer` (composition-root injection, `backend.cjs:229`) and the 8 port contract modules (`persistence-port, scene-data-port, placeholder-audio-port, progress-events-port, audio-fsm-port, video-fsm-port, hub-cancel-port, layer-config-port`) consumed by `backend.cjs` + the 8 storage adapters. **Internal:** everything else in the move set (engines, FSM glue, lease/counter/circuit-breaker/retry, scene-window, scheduler, worker-health, metrics) is NOT exported at package root. **Host-only composition:** `runtime/orchestration-seams.js` (stays host-side, binds the package's public API), `runtime/index.js` facade re-point, `gpu-dispatcher` implements `ports.dispatchTransport`, OUTPUT_DIR injection (step 4a). **Test-only bindings:** `tests/helpers/wire-seams.js` + seam-registry consumers stay host-side; require.cache resets re-point to the package specifier in the same commit. No host implementation detail ships inside the package: adapters, state, config, media, routes all stay in backend.

#### Dependency direction after the move (proven by closure, not asserted)

`backend.cjs (host) → @animastor/orchestration → {ports(in-package), host-stays via injected adapters}` — the closure scan found **zero move-set→host edges outside the allowlist**, so after `git mv` + re-point the graph still resolves with runtime→orchestration = 0 preserved (the tier pair becomes ONE package), no orchestration→host-implementation edge, no orchestration→book/storage/services edge (book = 0 measured; storage/services reachable only through injected adapters/ports). **No new SCC is possible from the move:** all inbound host→package edges (backend.cjs ×3, generation-routes ×4 lazy, book/generation-routes, task-handler ×2, video-orchestrator, iu-processor ×2, video-service, postgres/schema, state ×2, 8 adapters) are one-directional host→package and become package-specifier imports; the single pre-existing SCC pair stays host-side in services/**.

#### Physical-move step plan (NOT executed here)

1. **Pre-clean commit:** drop the dead `initializeRuntime` export from `runtime/runtime-scheduler.js` (MG-G-verified: one require site, zero callers); verify MG suite green.
2. **Move commit:** `git mv backend/src/runtime/{21 files}` → `packages/animastor-orchestration/src/runtime/`, `git mv backend/src/orchestration/{7 files}` → `packages/animastor-orchestration/src/orchestration/`; create `packages/animastor-orchestration/package.json` (`name: @animastor/orchestration`, `main: src/index.js`, dep `@animastor/generation` + `@animastor/contracts`); root `src/index.js` re-exports the facade + `createRuntimeResultConsumer` + the 8 port modules; `ports/` re-export layer preserves the intra-package relative specifiers unchanged (no file-content edits needed for static requires).
3. **Re-point commits (mechanical, no logic):** backend.cjs ×3 (`./orchestration`→`@animastor/orchestration`, `./runtime`→`@animastor/orchestration/runtime/...` per facade map, runtime-result-consumer); routes ×6 lazy sites (scene-window, worker-health ×2, job-schema ×2, dispatch-engine ×2 incl. book/); services ×4 (task-handler ×2, video-orchestrator, iu-processor ×2 via `@animastor/orchestration` subpaths); `video/video-service` job-schema→`@animastor/contracts`; `postgres/schema.js` + `audio/chunks.js` references; `runtime/index.js` facade re-points internals to the package; `runtime/job-schema` facade retained (host lazy consumers) or deleted with consumers re-pointed; `runtime/index.js` facade re-points internals to the package; test helpers `wire-seams.js` + require.cache resets re-point to the package specifier; workspace `package.json` gets the new workspace entry.
4. **Composition-root wiring:** (4a) inject OUTPUT_DIR into the package (kills the 3 O-G9 env reads — host passes `config.OUTPUT_DIR`); (4b) inject the workspace resolver: `scene-window`'s single lazy `gpuDispatcher.resolveWorkspaceForBook` call site (MG-F-frozen) becomes a host-supplied `resolveWorkspaceForBook` injected at the seams registry, backend.cjs passes `(bookId) => gpuDispatcher.resolveWorkspaceForBook(bookId)` with the same optional-load try/catch semantics preserved host-side.
5. **Interim shim phase (optional, ≤1 commit):** thin `backend/src/runtime/*-port.js` re-export shims so `state/**` port-consumer requires keep working; delete shims when state re-points. MG-A's HOST_STAYS already anticipates this phase.
6. **Cleanup commit:** delete `runtime-persistence.js` + `retention-manager.js`; delete shim files; remove `runtime/index.js` barrel if fully re-pointed.
7. **Tests that move:** `tests/counter-reconciliation*.test.js` (pure policy, zero host deps — re-point to package specifier); **tests that stay:** all `tests/architecture/**` (they guard the HOST boundary — re-pointed path constants only, per the MG suite header note), all dispatch/lease/runtime suites that bind via wire-seams (host integration), state/storage/media suites. Rollback: the move commit is a pure `git mv` + specifier re-point — `git revert` of the 2–3 move commits restores backend/src exactly; no data, schema, or protocol change is involved.

#### Guard suite — `backend/tests/architecture/physical-move-gate.test.js` (8 guards, all born-green)

MG-A move-set closure (static+lazy, zero edges outside the 31-target allowlist) · MG-A2 the seven classified runtime stays are exactly classified and no new runtime file can silently join · MG-B computed-require proximity scan (S5-A convention) · MG-C host composition edges stay host-owned (seams registry present, gpu-dispatcher PG-repo pins intact, runtime-loop present, result-consumer injected not required) · MG-F the PW-2 scene-window→gpu-dispatcher exception frozen at exactly one lazy call site, no other move-set file touches the transport · MG-G the dead runtime-persistence surface frozen (one require site, one consumer fn, zero callers repo-wide) · MG-D zero Generation deep imports (O-G6 parity for the future package) · MG-E no `__dirname`/`require.cache` in the move set (comment-stripped).

#### Test baseline (exact, measured at this branch)

`test:arch` before this step: **939 passing / 0 failing** (= §32.28 baseline). After adding the gate: **947 passing / 0 failing**. Full backend suite: **3385 passing / 13 failing — exactly the documented environment-dependent set** (redis/PG/network-dependent suites per §32.27's classified list). Generation package tests: green. Flaky-class note: `tests/lac-contract-sync` (LAC-6) failed 1 of 2 combined runs, green solo — network-class, matches the documented flaky set. **No GitHub CI status available: the remote is local-only (`/home/animastor/repos/animastor.git`), no GitHub workflows exist.**

...
**Phase-4 implementation note (2026-09-02):** non-streaming inference is
implemented exactly on the §4/§5 frames — `S→C chat.request
{ request_id, model, messages[{role,content}], params{max_tokens,
temperature}, timeout_ms }`, `C→S chat.response { request_id, model,
content, finish_reason, usage }`, `C→S chat.error { request_id, code[,
message] }`, `S→C chat.cancel { request_id }`. Fixed contract:

- **Limits (server-enforced on the connector; mirrored defensively on the
  cloud before sending; a hostile frame never executes):** `request_id`
  printable string ≤ 128; `model` string 1–512 chars, no control
  characters; `messages` array of 1–64 entries, `role` ∈
  {system,user,assistant}, `content` string 1–32 768 chars each, total
  prompt ≤ 131 072 chars; `params.max_tokens` integer 1–8192,
  `params.temperature` number 0–2 (any other params key is dropped);
  `timeout_ms` clamped to 1 000–180 000 (the cloud window, §5); whole
  chat.request frame ≤ 1 MB; runtime response body ≤ 1 MB; response
  `content` ≤ 32 768 chars and the serialized chat.response frame ≤ 60 KB
  (fits the 64 KB inbound frame cap — an ordinary long completion fails
  with a sanitized error instead of killing the session).
- **chat.error allowlist (sanitized, fixed):** `invalid_request`,
  `request_too_large`, `model_not_found`, `busy`, `timeout`,
  `runtime_unreachable`, `context_length`, `bad_response`,
  `runtime_error`, `response_too_large`. Size violations map to
  `request_too_large`, shape/type violations to `invalid_request`. No raw
  runtime error text ever crosses the WS; `message` is a fixed sanitized
  string per code (control-stripped, ≤ 256 chars on the cloud side).
- **Model validation:** presence/format/size only. NO hard gate against
  the last discovered model list (discovered ≠ loaded, §7 — a stale list
  would false-reject cold models); no `/v1/models` probe before inference
  (AD-7). The runtime stays the authority: its 404 → `model_not_found`;
  its context-overflow error → `context_length`.
- **stream is always false in Phase 4** — the adapter hardcodes
  `stream:false` in the runtime request body regardless of any frame
  field; `chat.delta` does not exist yet (Phase 5).
- **Concurrency:** per-session semaphore, default 2 (§4); overflow
  answers `chat.error{code:'busy'}` immediately. `request_id` is
  generated by the cloud (UUID); a duplicate request_id (in-flight OR
  already executed within the session lifecycle) is rejected
  `invalid_request` and never re-executed. The connector's per-session
  seen-id store keeps a fingerprint of EVERY request_id it has seen
  (admitted, busy-rejected or invalid) and never evicts entries; when the
  store reaches its bound (`maxSeenRequestIds`, 100 000) the session turns
  fail-closed and refuses every NEW request_id with `invalid_request`
  until the lifecycle ends (close/stop clears the store; a reconnect
  starts a fresh lifecycle). Memory stays bounded without ever weakening
  at-most-once.
- **Cancellation (§5):** the cloud timer is authoritative — on expiry the
  transport sends `chat.cancel` and fails the caller with `timeout`; the
  connector aborts the local fetch (AbortController), frees the slot and
  sends NOTHING for a cloud-cancelled request (late terminal frames for
  cancelled/unknown ids are dropped by the cloud). A connector-side
  timeout answers `chat.error{code:'timeout'}`. Cancel for an
  unknown/finished id is ignored silently; cancellation never closes the
  session; local logging records status `cancelled` (AD-6 metadata only).
- **Cloud transport:** `ai-connector/transport.js` — the callAI-shaped
  seam: validates against the same limits, looks up the live session in
  the registry (offline → fail-closed explicit `connector_offline`, no
  fallback, AD-12), multiplexes pending requests by `request_id`
  (session-bound), converts cloud-timeout → `chat.cancel` + `timeout`,
  fails pending fast with `session_closed` when the socket dies, drops
...
**Phase-4 implementation note (2026-09-02, scope split):** implemented
in two steps. Step 1 (this phase, done): the full WS inference pipeline —
connector-side `chat.request → runtime → chat.response/chat.error` with
limits/concurrency/cancellation (§4 Phase-4 note, AD-5/AD-6/AD-7 intact)
plus the cloud transport seam `ai-connector/transport.js` (request
multiplexer, authoritative timeout → `chat.cancel`, sanitized error
surface) and the WS route's `chat.response`/`chat.error` handling; the
pipeline is exercised end-to-end by the test suite. Step 2 (deferred):
the resolver/provider wiring above (`workspace-ai-provider.js`
local-ai branch, `ai-service.js`/`ai-routes.cjs` transport branches,
`settings-ai-routes.cjs`) — deliberately NOT touched in step 1; the §9

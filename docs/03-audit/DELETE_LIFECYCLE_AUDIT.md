...
| # | Severity | Component | Current | Expected | Risk | Fix (direction) |
|---|---|---|---|---|---|---|
| **C1** | **Critical** | `entity-crud-routes.cjs` + `book-sync.js` | Deleting scene/chapter/unit does not remove PG rows. `purgeRemovedSceneRows` only called from `PUT /book` and for entity-delete does not physically detect "removed" (JSON already saved). Even when called, it does not cover `asset_states`, `cache_entries`, `output_manifests`, `book_events`, `book_source`, `book_generation_sessions`, `chat_*`, `sentence_resolutions`, `character_mentions`. | Entity deletion synchronously cleans all its PG rows (following `DELETE /book` pattern). | Data leakage, table growth, "ghost" tasks/assets in statuses and metrics; version-stale checks see deleted scenes. | Extract scene/chapter-level purge (analogous to `purgeRemovedSceneRows`, expand table list) and call from DELETE handlers after `saveBookBundle`. |
| **C2** | **Critical** | Redis + `chunks-routes.cjs` | Chunks, per-asset states, active-index, asset-registry, dispatch/audio-orch keys of deleted scene not cleaned. `GET /chunks` and `/assets-state` build queue/status from Redis without JSON filter → deleted scene remains in playback queue (web+Android) and statuses. | Scene deletion removes its Redis keys and excludes from queue/statuses. | User "sees" and can play deleted scenes; UI confusion; Redis growth. | Scene-level Redis cleanup (chunks+set, asset-state, active-index, registry, audio-orch, iu-progress/in-flight, dispatch) + `GET /chunks` filter by book JSON. |
| **C3** | **Critical** | Dispatch / workers | On deletion, in-flight dispatch (leases), GPU jobs and cancel flags are not cancelled. Workers finish writing files+PG+Redis for deleted scene. | Deletion cancels in-flight generation for deleted scenes (like `DELETE /book`: cancelled-workers + setCancelFlag + clearHubDispatches). | Orphan files and rows, unnecessary GPU costs, "resurrection" of deleted scene state via callbacks. | In DELETE handlers: `clearLeasesForScenes`/`clearHubDispatches` for deleted scenes, `setCancelFlag`, GPU `/queue/clear` with scene filter. |
| **H1** | **High** | filesystem-store / cleanup-service | Files of deleted scene/chapter (mp3, chunks, `_iu*`, `_pr*`, mp4) remain in OUTPUT_DIR forever; no scene-level GC. | Scene deletion removes its files; background sweep cleans orphan files (no references from JSON). | Disk waste, confusion risk during recovery, extra space for "resurrection". | Delete files by prefix `{bookId}_{ch}_{sc}`/`{bookId}_{ch}` on delete; or background GC by book JSON. |
| **H2** | **High** | generation-routes.cjs / resetScenes | Pre-delete stale PNG (dirty-unit path) only works for `/regenerate` via `cleanPngUnitIds`; not executed for entity-delete. IU/preview PNG of deleted unit remain. | Unit deletion removes its `_iu<uid>.png`/`_pr<uid>.png` and cleans `image_units`/`scene_assets` rows. | Orphan images, id reuse excluded, but waste grows. | Add unit-level cleanup (files + PG) to DELETE unit. |
| **M1** | **Medium** | runtime-persistence / reconciliation | Runtime snapshot (leases/dispatch metadata/retry) can "restore" active state of deleted scene after restart (snapshot taken before deletion). Recovery does not clean stale data of deleted scenes. | After restart, deleted scenes do not return to runtime. | Temporary "ghost" in runtime until self-heal. | In reconcile/startup, check snapshot scenes against book JSON and discard missing ones. |
| **M2** | **Medium** | mediaCache.ts (web) / Repository.kt + SimpleDiskCache + VideoCache (Android) | ✅ **Fixed** (§7). Web: `evictSceneMedia/evictChapterMedia` + `clearPreloadCache` + queue. Android: `clearCache()` + `removeDeletedScenesFromQueue`. | — | — | — |
| **M3** | **Medium** | book-diff / book-sync / generateStore | Entity-delete does not mark affected scenes dirty and does not bump versions; dirty indicator and regeneration only exist via `/regenerate` (rebuild_all). | Unit/scene deletion marks neighboring/affected scenes dirty (coordinated regen). | UI shows "clean" though structure changed; statuses "ghost" until next generation. | Call diff/version-bump after delete (following `PUT /book` pattern). |
| **M4** | **Medium** | asset-registry.js | `deleteChapterAssetsRedis`/`deleteBookAssetsRedis` call `storage.scanKeys(...)`, which does not exist in filesystem-store → TypeError on call. None of the three delete functions are called anywhere. | Scene/chapter/book asset-registry cleanup works and is called. | Latent bug, "dead" code. | Implement `scanKeys` or use `redis.scan`; call from DELETE handlers (see C2). |
| **M5** | **Medium** | EditPage.tsx / EditFragment.kt / playbackStore | ✅ **Fixed** (§7). Web: `invalidateDeletedScene/Chapter` updates queue + `needsContentRefresh`. Android: `removeDeletedScenesFromQueue` + `reloadStructureAndReposition`. | — | — | — |
| **L1** | **Low** | book/index.js | Orphan file cleanup for chapters only runs when `chapterFilenames.length > 0`; "last chapter" guard protects invariant. | — (invariant holds). | No practical risk; worth codifying invariant in code. | — |
| **L2** | **Low** | redis-helpers / recover-chunks | `recover*` only adds (creates) chunks/placeholders from book JSON, never removes stale data of deleted scenes. | — (does not resurrect, but does not clean either). | Amplifies C2/H1: stale data accumulates. | Sweep stale chunks/files in reconcile (see C2/H1). |
| **L3** | **Low** | scene-orchestrator.js | `loadedBook || book.loadBook(bookId)` — if `loadedBook` is stale (tick cache), self-heal may not fire until next tick. | — (self-heal usually fires). | Brief extra dispatch cycle. | Refresh book cache before dispatch. |
